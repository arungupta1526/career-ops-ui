...
基于 v1.6.0 QA 报告的 21 次提交的安全加固 + UX 改进。落地了三层安全 (XSS 净化、CSP、输入校验),补齐了缺失的 CRUD 端点 (DELETE jds、POST tracker),UI 新增两个页面 — **Activity** 和重新设计的 **Deep Research**(在浏览器中直接运行)。测试覆盖率从 **73** 增加到 **177**,新增 **14 个测试文件**。
