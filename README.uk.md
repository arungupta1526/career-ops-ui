...
> **🆕 Останній реліз — v1.78.1**
>
> **Покращення UX сканування.** Таблиця результатів на `#/scan` тепер **автоматично оновлюється** в реальному часі під час сканування та ще раз після його завершення — без ручного перезавантаження. Глобальний пошук у верхній панелі показує підказку **Enter**, і за запитом, що не є URL, переходить на `#/scan` із **попередньо заповненим** полем пошуку (раніше — `#/tracker`); **логотип бренда веде на дашборд** (головну). Базується на v1.78.0 (**гео-фільтр сканування** — фільтрація результатів за країною з прапорцями, поряд із Remote / Hybrid / Onsite), v1.77.0 (данська, 13-та локаль) і v1.76.0 (паритет із батьківським career-ops v1.13.0 — шість джерел ATS на тенант, `trust_filter`, необмежені результати сканування).
>
