...
[![tests](https://img.shields.io/badge/tests-1222%20passed-brightgreen)](#тести)
...
> **🆕 Останній реліз — v1.78.0**
>
> **Гео-фільтр на сторінці сканування — фільтруйте результати вакансій за країною, із прапорцями.** Новий випадаючий список **Country** на `#/scan` перелічує кожну країну, виявлену у ваших відсканованих результатах (емодзі прапорця + лічильник), щоб залишити лише вакансії, прив'язані до конкретної країни — поряд із фільтром типу роботи Remote / Hybrid / Onsite, тож ви можете шукати як роботу, прив'язану до країни, **так і** віддалену. Новий хелпер `countries.js` зіставляє вільний текст локації кожної вакансії (назви країн, аліаси та ~100 великих міст ринку праці) з ISO-країною + прапорцем; виявлення консервативне й ніколи не вгадує. Базується на v1.77.0 (данська, 13-та локаль) і v1.76.0 (паритет із батьківським career-ops v1.13.0 — шість джерел ATS на тенант, `trust_filter`, необмежені результати сканування).
>
