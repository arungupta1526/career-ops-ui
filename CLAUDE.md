...
- **Versioning:** `package.json` is the source of truth (currently 1.58.11). The footer reads it via `/api/health`. The parent's `VERSION` file is reported separately as `parentVersion` — they drift independently.
