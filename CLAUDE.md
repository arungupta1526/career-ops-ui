...
8. **Help bundle parity (H2 + H3)** is locked by `tests/{canonical-docs-coverage,help-ru-config-section,help-ui}.test.mjs`. As of v1.60.0: **19** H2 sections (§19 "Localizing the app" added; was 18 pre-§19), **73** H3 subsections. Adding a new H2 section means bumping the count in `canonical-docs-coverage` + `help-ui`; adding an H3 means bumping `help-ru-config-section`.
