...
> **🆕 最新リリース — v1.67.1**
>
> **1. `#/scan` の給与レンジフィルタ。** 結果テーブルに、テキスト・リモートのフィルタの隣に2つの数値入力（**給与 下限 / 上限**）が加わりました。各行のフリーテキスト給与（`от 100 000 до 200 000 ₽`、`120000-150000 USD`、`$120K–$150K` など）を数値レンジに解析し、レンジの重なり判定でマッチします — 行の給与レンジが `[下限, 上限]` の範囲と重なれば残ります。**給与未掲載の行は表示され続ける**ため、フィルタは一覧を絞り込むだけでゼロにはしません。比較は**通貨を区別しません**（為替換算なし）。
>
> **2. ソース別フェッチのタイムアウトを 10s に設定（fail-fast）。** v1.67.0 で 30s に上げても遅い Ashby ボードの半分ほどしか復活せず、残り（Perplexity・Supabase・Resend・DeepL・Ramp など）は期限に関係なくハングするため、長いタイムアウトは死んだ枠を待ってスキャンを停滞させるだけでした。10s は慢性的なハングに即座に失敗し、スキャンの応答性を保ちます。`SCAN_FETCH_TIMEOUT_MS` で上書き可。
>
> _フルスイート **1060/1060** グリーン · i18n + ドキュメントを全9ロケールで同期。_
