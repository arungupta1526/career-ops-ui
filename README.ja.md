...
>
> **2. ソース別フェッチのタイムアウトを 15s → 30s に延長**（`SCAN_FETCH_TIMEOUT_MS` で上書き可）。Ashby の `includeCompensation` ペイロードは ×8 並列下で 15s を超えることが多く、毎回 ~30 の Ashby ボードがタイムアウトしていました。30s で遅いが生きているソースが完了できます。
>
> _テスト13件追加 · フルスイート **1060/1060** グリーン · i18n + ドキュメントを全9ロケールで同期。_
