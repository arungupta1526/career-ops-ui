...
> **🆕 最新リリース — v1.83.0**
>
> **再掲載 / ゴースト求人検出器**（親 career-ops v1.15.0 パリティ）: `#/scan` に新しい **🔁 再掲載 / ゴースト求人** パネルが追加され、90 日のローリングウィンドウ内で異なる URL に再掲載された企業+職種クラスタを表示します — 古い採用パイプライン / ゴースト求人のシグナル — スキャン履歴から読み込みます（`GET /api/scan/reposts`）。v1.82.0（NoDesk）、v1.81.0（13 件の新規ソース）、v1.80.0（Teamtailor、ソース隔離、保存済み検索）をベースにしています。
>
> _13 ロケール · 6 LLM プロバイダ · 41 スキャナアダプタ · 再掲載検出器 · 親 career-ops v1.15.0 パリティ。_
