...
> **🆕 最新リリース — v1.81.0**
>
> **13 件の新規求人ボードスキャンソース**（親 career-ops パリティ）: ボード全体の公開 API **Arbeitnow · Himalayas · Jobicy · Landing.jobs · 4 Day Week · The Muse · The Hub · Jobspresso · Hacker News "Who is hiring?"**、ポーランドのボード **JustJoin.it · NoFluffJobs**（POST 検索）、テナント別 ATS **Pinpoint · Rippling**（`careers_url` から自動検出）— すべて `redirect:'error'` でホスト固定（SSRF 対策）され、`#/scan` の **Source** ドロップダウンで選択できます（**40 アダプタ**合計）。v1.80.0（Teamtailor、ソース隔離、ソースごとの上限、期間フィルター、保存済み検索）、v1.79.0（We Work Remotely）、v1.77.0（デンマーク語）、v1.76.0（テナント別 ATS ソース）をベースにしています。
>
> _13 ロケール · 6 LLM プロバイダ · 40 スキャナアダプタ · 13 件の新規ソース · 親 career-ops パリティ。_
