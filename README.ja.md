...
> **🆕 最新リリース — v1.84.0**
>
> **再応募クールダウン + pipeline.md への報酬情報**（親 career-ops v1.15.0 パリティ）: EN スキャンが**最近応募した企業の求人をスキップ**するようになり（`config/profile.yml → re_apply_windows:`、デフォルト無効）、結果が新しい求人にフォーカスされます。また、スキャンされた求人の**給与**がオプションの `url | <comp>` 列として `data/pipeline.md` に保存されます。v1.83.0（再掲載検出器）、v1.82.0（NoDesk）、v1.81.0（13 件の新規ソース）をベースにしています。
>
> _13 ロケール · 6 LLM プロバイダ · 41 スキャナアダプタ · 再掲載検出器 · 再応募クールダウン · 親 career-ops v1.15.0 パリティ。_
