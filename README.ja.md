...
> **v1.67.0 — `#/scan` に給与レンジフィルタを追加。** 結果テーブルに、テキスト・リモートのフィルタの隣に2つの数値入力（**給与 下限 / 上限**）が加わりました。各行のフリーテキスト給与（`от 100 000 до 200 000 ₽`、`120000-150000 USD`、`$120K–$150K` など）を数値レンジに解析し、レンジの重なり判定でマッチします。給与未掲載の行は表示され続けるため、フィルタは一覧を絞り込むだけでゼロにはしません（通貨は区別せず＝為替換算なし）。さらにソース別フェッチのタイムアウトを **15s → 30s** に延長し、遅い Ashby ボードのタイムアウトを解消。フルスイート **1060/1060** グリーン。
