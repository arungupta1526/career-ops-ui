...
> **🆕 最新リリース — v1.68.2**
>
> **`npx` / `npm link` 経由の CLI 動詞が壊れていた — 修正しました。** npm と npx は `career-ops-ui` を `node_modules/.bin/` 配下のシンボリックリンクとして公開しますが、従来のパス処理はパッケージのルートではなく `.bin` を指していたため、`npx career-ops-ui init` が `MODULE_NOT_FOUND` で落ちていました。`bin/career-ops-ui.sh` と `bin/start.sh` はシンボリックリンクのチェーンをたどってスクリプトのパスを解決するようになり（`readlink` + `cd -P`）、すべての動詞がリポジトリから、`npm link` 経由、`npx` 経由で動作します。`.bin` スタイルのシンボリックリンク経由で動詞を実行する新しい回帰テストで固定しました。
>
> _フルスイート **1065/1065** グリーン · i18n + ドキュメントを全9ロケールで同期。_
