...
> **🆕 最新リリース — v1.82.0**
>
> **NoDesk スキャンソース**（親 career-ops v1.15.0 パリティ）: ボード全体の NoDesk リモート求人 RSS フィードが第一級のスキャンソースになりました — `provider: nodesk` のエントリを追加すると `#/scan` の **Source** ドロップダウンに表示されます（合計 **41 アダプタ**）。ホストは `nodesk.co` に固定し `redirect:'error'`（SSRF 対策）；タイトルは `Role at Company` で分割。v1.81.0（13 件の新規ソース）、v1.80.0（Teamtailor、ソース隔離、保存済み検索）、v1.77.0（デンマーク語）をベースにしています。
>
> _13 ロケール · 6 LLM プロバイダ · 41 スキャナアダプタ · 親 career-ops v1.15.0 パリティ。_
