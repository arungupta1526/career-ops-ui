...
> **🆕 最新リリース — v1.69.0**
>
> **プラグイン式スキャナ自動検出 (P-14) — `server/lib/sources/` に `.mjs` を置くだけで自動登録されます。** v1.69 以前、`server/lib/sources/registry.mjs` のソースリストは手動管理の配列でした。アダプタを追加するには `<id>.mjs` と `registry.mjs` の両方を編集する必要がありました。今やすべてのアダプタが自己記述型の `export const meta = { value, label, region, configKey? }` ブロックを宣言し、レジストリが起動時に自動検出します（`readdirSync` + 動的 `import()`）。ロードマップ項目 P-14 の前半を完了。公開 API（`SOURCES`、`SOURCES_BY_REGION`、`RU_CONFIG_KEYS`、`getRegionalSources`）は変更なし — 既存のインポートはすべて動作し続け、不正な `meta` は診断警告 1 件でスキップされます。ヘルプ §17 (「新しい求人ポータルソースを追加する方法」) を全 9 ロケールでドロップインフローに合わせて書き直しました。
>
> _フルスイート **1079/1079** グリーン · i18n + ドキュメントを全 9 ロケールで同期。_
