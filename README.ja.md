...
> **🆕 最新リリース — v1.69.1**
>
> **2 件の修正。** **(1) scan:** `#/scan` のテーブルが大規模な地域スイープを黙って切り詰めなくなりました — 表示セットがリージョンごとに 500 件で固定されていました（一致 1352 件の RU スキャンが 500 件しか表示せず、852 件が非表示）。両スキャナーは `MAX_STORED_RESULTS`（既定 2000、`SCAN_MAX_RESULTS` で設定可）を使用します。表示のみ。**(2) health/ui:** `#/health` のチェックカードがはみ出さなくなりました — 長い名前／値が Fix ボタンとバッジに衝突していましたが、`.health-check-row` で縮小・折り返しします。
>
> _フルスイート **1084/1084** グリーン · i18n + ドキュメントを全 9 ロケールで同期。_
