...
> **🆕 最新リリース — v1.70.0**
>
> **3つの新言語 — Polski, Українська, العربية (RTL) — が追加され、UIは12ロケールになり、親プロジェクトのcareer-ops READMEと同等になりました。** アラビア語は本格的な右から左への表示をサポートします。言語選択はフラグ付き `<select>` ドロップダウンになり、親プロジェクトのカバーレターモードが `#/cover` に追加されました。親プロジェクト career-ops v1.11.0 に対応しています。
>
> _全テスト **1086/1086** グリーン · 12ロケール · ドキュメント同期済み。_
