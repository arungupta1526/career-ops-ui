...
> **🆕 最新リリース — v1.79.0**
>
> **WeWorkRemotely スキャンソース(親 career-ops v1.14.0 パリティ)。** ボード全体を対象とする [We Work Remotely](https://weworkremotely.com) のリモート求人 RSS フィードが、第一級のスキャンソースになりました — `provider: weworkremotely` のエントリを追加すると `#/scan` の **Source** ドロップダウンに表示されます(合計 **26 アダプタ**)。ホスト固定 + `redirect:'error'`(SSRF 安全)、タイトルは `Company: Role` で分割されます。さらに `title_filter` のキーワードは長さチェックの前にトリムされます(親 #1261)。v1.78.x(スキャンの**国フィルター**、結果の**自動更新**、グローバル検索の **Enter→スキャン**、クリック可能なロゴ)、v1.77.0(デンマーク語、13 番目のロケール)、v1.76.0(テナント単位の ATS ソース 6 件、`trust_filter`、上限なしのスキャン)をベースにしています。
>
> _13 locales · 6 LLM プロバイダ · 26 スキャナアダプタ · 国フィルター · 親 career-ops v1.14.0 パリティ。_
