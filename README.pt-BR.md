...
[English](README.md) | [Español](README.es.md) | **Português (Brasil)** | [한국어](README.ko-KR.md) | [日本語](README.ja.md) | [Русский](README.ru.md) | [简体中文](README.zh-CN.md) | [繁體中文](README.zh-TW.md) | [Français](README.fr.md) | [Polski](README.pl.md) | [Українська](README.uk.md) | [Dansk](README.da.md) | [العربية](README.ar.md)

_UI não oficial — sem afiliação ou endosso de career-ops / santifer._

[![tests](https://img.shields.io/badge/tests-1244%20passed-brightgreen)](#testes)
...
> **🆕 Última versão — v1.79.0**
>
> **Fonte de varredura WeWorkRemotely (paridade com o career-ops pai v1.14.0).** O feed RSS de vagas remotas de todo o quadro do [We Work Remotely](https://weworkremotely.com) agora é uma fonte de varredura de primeira classe — adicione uma entrada `provider: weworkremotely` e ela aparece no menu suspenso **Fonte** de `#/scan` (**26 adaptadores** no total). Fixado ao host + `redirect:'error'` (à prova de SSRF); os títulos são divididos por `Company: Role`. Além disso: as palavras-chave de `title_filter` são aparadas antes da verificação de comprimento (pai #1261). Baseia-se na v1.78.x (filtro por **país** na Varredura, **atualização automática** de resultados, **Enter→Varredura** na busca global, logo clicável), na v1.77.0 (dinamarquês, 13.º idioma) e na v1.76.0 (seis fontes ATS por tenant, `trust_filter`, varredura sem limite).
>
> _13 locales · 6 provedores LLM · 26 adaptadores de scanner · filtro por país · paridade com o career-ops pai v1.14.0._
...
O [career-ops](https://career-ops.org) é um sistema open-source de busca de emprego que roda como slash commands dentro de qualquer CLI de IA para código (Claude Code, Gemini CLI, Codex, Qwen Code, OpenCode, GitHub Copilot CLI, Antigravity CLI — outras CLIs compatíveis com Claude também funcionam pela mesma superfície de slash-commands). É agnóstico em relação ao modelo. Avalia cada vaga contra o seu CV usando uma rubrica de seis dimensões na escala 0.0–5.0, gera currículos em PDF personalizados e registra cada candidatura localmente — sem contas em nuvem, sem telemetria, sem envio automático.
