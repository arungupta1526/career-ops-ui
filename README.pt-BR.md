...
[![tests](https://img.shields.io/badge/tests-1079%20passed-brightgreen)](#testes)
...
> **🆕 Última versão — v1.69.0**
>
> **Auto-descoberta de fontes do scanner como plug-in (P-14) — basta soltar um `.mjs` em `server/lib/sources/` e ele se registra sozinho.** Antes da v1.69 a lista de fontes em `server/lib/sources/registry.mjs` era um array mantido manualmente: adicionar um adapter significava editar tanto `<id>.mjs` quanto `registry.mjs`. Agora cada adapter declara um bloco auto-descritivo `export const meta = { value, label, region, configKey? }`, e o registry o auto-descobre na inicialização (`readdirSync` + dynamic `import()`). Fecha a metade pendente do item P-14 do roadmap. A API pública (`SOURCES`, `SOURCES_BY_REGION`, `RU_CONFIG_KEYS`, `getRegionalSources`) não mudou — todo import existente continua funcionando, e um `meta` malformado é ignorado com um único aviso de diagnóstico. A seção §17 do Help ("Como adicionar uma nova fonte de portal de vagas") foi reescrita nos 9 idiomas para o fluxo drop-in.
>
> _Suíte completa **1079/1079** verde · i18n + docs sincronizados nos 9 idiomas._
