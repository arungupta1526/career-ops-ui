...
> **v1.66.0 — as fontes RU agora percorrem todas as páginas de resultados.** hh.ru, Habr Career e Trudvsem paginavam apenas os primeiros ~50 resultados por consulta; agora seguem a paginação até o fim — `&page=N` para hh.ru/Habr, `offset`/`meta.total` para Trudvsem — deduplicando entre páginas e parando quando uma página não traz nada novo (ou no limite de segurança de 50 páginas). Uma consulta como «Backend разработчик» retorna agora o conjunto completo em vez de uma só página (ex.: hh.ru PHP **17 → 55+** em 3 páginas; Trudvsem retorna os 72). Cada página mantém o timeout + `AbortSignal` existentes. Suíte completa **1045/1045** verde.
