...
[![tests](https://img.shields.io/badge/tests-1258%20passed-brightgreen)](#testes)
...
> **🆕 Última versão — v1.80.0**
>
> **Cinco melhorias na varredura** (ideias do [job-crawler](https://github.com/bracketouverte/job-crawler), reimplementadas): uma fonte **Teamtailor** — sites `<slug>.teamtailor.com` por tenant via o feed público `/jobs.rss` deles (**27 adaptadores** no total); **quarentena de fontes** — fontes mortas 404/410 são registradas e ignoradas nas varreduras seguintes (autorrecuperação após 14 dias), eliminando o ruído recorrente de slugs mortos; um campo de limite **Máx. por fonte** (∞ por padrão); um filtro de idade **Publicado nos últimos** (24h / 7d / 30d); e **buscas salvas + favoritos ★** em `#/scan`, persistidos em `localStorage` com validação defensiva. Baseia-se na v1.79.0 (We Work Remotely), na v1.78.x (filtro por país, atualização automática, Enter→Varredura, logo clicável), na v1.77.0 (dinamarquês) e na v1.76.0 (fontes ATS por tenant, `trust_filter`).
>
> _13 locales · 6 provedores LLM · 27 adaptadores de scanner · buscas salvas + favoritos · paridade com o career-ops pai v1.14.0._
