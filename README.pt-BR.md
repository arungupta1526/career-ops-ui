...
[![tests](https://img.shields.io/badge/tests-1065%20passed-brightgreen)](#testes)
...
> **🆕 Última versão — v1.68.2**
>
> **Os verbos da CLI via `npx` / `npm link` estavam quebrados — agora corrigido.** npm e npx expõem `career-ops-ui` como um link simbólico em `node_modules/.bin/`, onde a lógica de caminho antiga apontava para `.bin` em vez da raiz do pacote, então `npx career-ops-ui init` falhava com `MODULE_NOT_FOUND`. Agora `bin/career-ops-ui.sh` e `bin/start.sh` resolvem o caminho do script através da cadeia de links (`readlink` + `cd -P`), de modo que cada verbo funciona a partir do repo, via `npm link` e via `npx`. Protegido por um novo teste de regressão que executa um verbo através de um link no estilo `.bin`.
>
> _Suíte completa **1065/1065** verde · i18n + docs sincronizados nos 9 idiomas._
