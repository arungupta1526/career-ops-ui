...
[![tests](https://img.shields.io/badge/tests-1084%20passed-brightgreen)](#testes)
...
> **🆕 Última versão — v1.69.1**
>
> **Duas correções.** **(1) scan:** a tabela de `#/scan` não trunca mais silenciosamente varreduras regionais grandes — o conjunto exibido estava limitado a 500 por região (uma varredura RU de 1352 vagas correspondentes mostrava apenas 500; 852 ocultas). Ambos os scanners agora usam `MAX_STORED_RESULTS` (padrão 2000, configurável via `SCAN_MAX_RESULTS`); apenas exibição. **(2) health/ui:** os cartões de `#/health` não transbordam mais — um nome/valor longo colidia com o botão Fix e o selo; a linha agora encolhe e quebra via `.health-check-row`.
>
> _Suíte completa **1084/1084** verde · i18n + docs sincronizados nos 9 idiomas._
