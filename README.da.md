...
> **🆕 Seneste udgivelse — v1.79.0**
>
> **WeWorkRemotely-scankilde (paritet med forælderens career-ops v1.14.0).** Det board-brede [We Work Remotely](https://weworkremotely.com)-RSS-feed til fjernjob er nu en førsteklasses scankilde — tilføj en `provider: weworkremotely`-post, og den dukker op i **Source**-dropdownen på `#/scan` (**26 adaptere** i alt). Host-pinned + `redirect:'error'` (SSRF-sikker); titler opdeles ved `Company: Role`. Desuden: `title_filter`-nøgleord trimmes nu før længdekontrollen (forælder #1261). Bygger videre på v1.78.x (scan-**landefilter**, resultater med **auto-opdatering**, **Enter→Scan** i den globale søgning, klikbart logo), v1.77.0 (dansk, 13. lokalitet) og v1.76.0 (seks ATS-kilder pr. tenant, `trust_filter`, scan uden loft).
>
> _13 lokaliteter · 6 LLM-providere · 26 scanneradaptere · landefilter · paritet med forælderens career-ops v1.14.0._
...
![career-ops-ui — Command Center](./images/dashboard-da.png)
