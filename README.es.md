...
> **v1.66.0 — las fuentes RU ahora recorren todas las páginas de resultados.** hh.ru, Habr Career y Trudvsem solo paginaban los primeros ~50 resultados por consulta; ahora siguen la paginación hasta el final — `&page=N` para hh.ru/Habr, `offset`/`meta.total` para Trudvsem — deduplicando entre páginas y parando cuando una página no aporta nada nuevo (o en un tope de seguridad de 50 páginas). Una consulta como «Backend разработчик» devuelve ahora el conjunto completo en vez de una sola página (p. ej. hh.ru PHP **17 → 55+** en 3 páginas; Trudvsem devuelve los 72). Cada página conserva el timeout + `AbortSignal` existentes. Suite completa **1045/1045** en verde.
