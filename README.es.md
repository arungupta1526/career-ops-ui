...
> **🆕 Última versión — v1.79.0**
>
> **Fuente de escaneo WeWorkRemotely (paridad con el career-ops padre v1.14.0).** El feed RSS de empleos remotos de todo el tablón de [We Work Remotely](https://weworkremotely.com) es ahora una fuente de escaneo de primer nivel — añade una entrada `provider: weworkremotely` y aparece en el desplegable **Fuente** de `#/scan` (**26 adaptadores** en total). Anclado al host + `redirect:'error'` (a salvo de SSRF); los títulos se dividen por `Company: Role`. Además: las palabras clave de `title_filter` se recortan antes de la comprobación de longitud (padre #1261). Se basa en la v1.78.x (filtro por **país** en Escaneo, **autoactualización** de resultados, **Enter→Escaneo** en la búsqueda global, logo clicable), la v1.77.0 (danés, 13.º idioma) y la v1.76.0 (seis fuentes ATS por tenant, `trust_filter`, escaneo sin límite).
>
> _13 locales · 6 proveedores LLM · 26 adaptadores de escáner · filtro por país · paridad con el career-ops padre v1.14.0._
