...
> **🆕 Última versión — v1.67.0**
>
> **1. Filtro de rango salarial en `#/scan`.** La tabla de resultados gana dos campos numéricos — **salario desde / hasta** — junto a los filtros de texto y remoto. El salario en texto libre de cada fila (`от 100 000 до 200 000 ₽`, `120000-150000 USD`, `$120K–$150K`, …) se parsea a un rango numérico y se compara con semántica de rangos solapados — una fila se conserva si su rango salarial se solapa con tu ventana `[desde, hasta]`. Las filas **sin salario publicado siguen visibles**, así el filtro acota en vez de vaciar la lista. La comparación es **independiente de la divisa** (sin conversión de cambio).
>
> **2. Timeout de fetch por fuente subido de 15s → 30s** (override con `SCAN_FETCH_TIMEOUT_MS`). Los payloads `includeCompensation` de Ashby tardaban habitualmente más de 15s con concurrencia ×8, así que ~30 tableros Ashby caían por timeout en cada escaneo; 30s deja terminar a las fuentes lentas pero vivas.
>
> _13 pruebas nuevas · suite completa **1060/1060** en verde · i18n + docs sincronizados en los 9 idiomas._
