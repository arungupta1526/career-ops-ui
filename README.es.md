...
> **🆕 Última versión — v1.69.1**
>
> **Dos correcciones.** **(1) scan:** la tabla de `#/scan` ya no trunca silenciosamente los barridos regionales grandes — el conjunto mostrado estaba limitado a 500 por región (un escaneo RU de 1352 ofertas coincidentes mostraba solo 500; 852 ocultas). Ambos escáneres usan ahora `MAX_STORED_RESULTS` (por defecto 2000, configurable con `SCAN_MAX_RESULTS`); solo afecta a la visualización. **(2) health/ui:** las tarjetas de `#/health` ya no se desbordan — un nombre/valor largo chocaba con el botón Fix y la insignia; la fila ahora se encoge y se ajusta vía `.health-check-row`.
>
> _Suite completa **1084/1084** en verde · i18n + docs sincronizados en los 9 idiomas._
