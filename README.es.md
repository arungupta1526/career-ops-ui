...
> **🆕 Última versión — v1.68.2**
>
> **Los verbos de la CLI vía `npx` / `npm link` estaban rotos — ya corregido.** npm y npx exponen `career-ops-ui` como un enlace simbólico bajo `node_modules/.bin/`, donde la lógica de ruta antigua apuntaba a `.bin` en lugar de a la raíz del paquete, así que `npx career-ops-ui init` fallaba con `MODULE_NOT_FOUND`. Ahora `bin/career-ops-ui.sh` y `bin/start.sh` resuelven la ruta del script a través de la cadena de enlaces (`readlink` + `cd -P`), de modo que cada verbo funciona desde el repo, vía `npm link` y vía `npx`. Asegurado con un nuevo test de regresión que ejecuta un verbo a través de un enlace estilo `.bin`.
>
> _Suite completa **1065/1065** en verde · i18n + docs sincronizados en los 9 idiomas._
