...
> **🆕 Última versión — v1.69.0**
>
> **Auto-descubrimiento de fuentes del escáner (P-14) — suelta un `.mjs` en `server/lib/sources/` y se registra solo.** Antes de v1.69 la lista de fuentes en `server/lib/sources/registry.mjs` era un array mantenido a mano: añadir un adapter implicaba editar tanto `<id>.mjs` como `registry.mjs`. Ahora cada adapter declara un bloque auto-descriptivo `export const meta = { value, label, region, configKey? }`, y el registro lo auto-descubre al arrancar (`readdirSync` + `import()` dinámico). Cierra la parte pendiente del ítem de roadmap P-14. La API pública (`SOURCES`, `SOURCES_BY_REGION`, `RU_CONFIG_KEYS`, `getRegionalSources`) no cambia — cada importación existente sigue funcionando, y un `meta` mal formado se omite con un único aviso de diagnóstico. La ayuda §17 ("Cómo añadir una nueva fuente de portal de empleo") reescrita en los 9 idiomas para el flujo drop-in.
>
> _Suite completa **1079/1079** en verde · i18n + docs sincronizados en los 9 idiomas._
