...
> **v1.62.1 — Adaptador RSS: escanea cualquier portal de empleo con RSS ([#15](https://github.com/Fighter90/career-ops-ui/pull/15)).** Un nuevo adaptador `rss` (`server/lib/portals/adapters/rss.mjs` + `server/lib/sources/rss.mjs`) permite al escáner extraer ofertas de cualquier feed RSS/Atom — **LaraJobs, WeWorkRemotely, RemoteOK, golangprojects** y otros portales fuera de Greenhouse/Ashby/Lever. Sin nuevas dependencias: el análisis del feed usa regex con CDATA y decodificación de entidades HTML (títulos/empresas sin etiquetas, code points astrales decodificados de forma segura). Actívalo por empresa con `provider: rss` + una clave `rss:` (o `feed_url:`) en `portals.yml` — sin cambios de código — y nunca intercepta empresas ya emparejadas con un ATS. **RSS** ahora aparece en el desplegable de filtro de fuentes de `#/scan` junto a Greenhouse / Ashby / Lever / Workable / SmartRecruiters / Workday. `ALL_ADAPTERS` crece 6 → 7. 29 pruebas nuevas; suite completa **1032/1032**. Docs + CHANGELOG en las 9 localizaciones.
