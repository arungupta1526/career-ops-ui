...
> **🆕 最新版本 — v1.68.2**
>
> **通过 `npx` / `npm link` 调用的 CLI 动词此前已损坏——现已修复。** npm 和 npx 会将 `career-ops-ui` 暴露为 `node_modules/.bin/` 下的符号链接，而旧的路径逻辑指向的是 `.bin` 而非包根目录，于是 `npx career-ops-ui init` 会以 `MODULE_NOT_FOUND` 崩溃。现在 `bin/career-ops-ui.sh` 与 `bin/start.sh` 会沿符号链接链解析脚本路径（`readlink` + `cd -P`），因此每个动词都能从仓库、通过 `npm link` 以及通过 `npx` 正常工作。已由一个通过 `.bin` 风格符号链接执行动词的新回归测试锁定。
>
> _完整套件 **1065/1065** 通过 · i18n 与文档已在全部 9 个语言同步。_
