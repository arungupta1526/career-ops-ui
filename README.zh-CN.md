...
> **🆕 最新版本 — v1.69.1**
>
> **两处修复。** **(1) scan：** `#/scan` 表格不再静默截断大型区域扫描 —— 显示集被硬限制为每区域 500 条（1352 条匹配的 RU 扫描只显示 500 条，隐藏 852 条）。两个扫描器现在使用 `MAX_STORED_RESULTS`（默认 2000，可经 `SCAN_MAX_RESULTS` 配置），仅影响显示。**(2) health/ui：** `#/health` 检查卡片不再溢出 —— 过长的名称/值会与 Fix 按钮和徽章重叠；该行现在通过 `.health-check-row` 收缩并换行。
>
> _完整套件 **1084/1084** 通过 · i18n 与文档已在全部 9 个语言同步。_
