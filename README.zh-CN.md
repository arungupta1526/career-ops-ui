...
> **🆕 最新版本 — v1.82.0**
>
> **NoDesk 扫描源**（与上游 career-ops v1.15.0 对齐）：NoDesk 全站远程职位 RSS 源现已成为一级扫描源 —— 添加 `provider: nodesk` 条目后即出现在 `#/scan` 的 **Source** 下拉中（共 **41 个适配器**）。主机锁定为 `nodesk.co` 并使用 `redirect:'error'`（防 SSRF）；标题按 `Role at Company` 拆分。在 v1.81.0（13 个新来源）、v1.80.0（Teamtailor、来源隔离、保存的搜索）和 v1.77.0（丹麦语）的基础上构建。
>
> _13 locales · 6 个 LLM 提供方 · 41 个扫描器适配器 · 与父级 career-ops v1.15.0 对齐。_
