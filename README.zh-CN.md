...
[English](README.md) | [Español](README.es.md) | [Português (Brasil)](README.pt-BR.md) | [한국어](README.ko-KR.md) | [日本語](README.ja.md) | [Русский](README.ru.md) | **简体中文** | [繁體中文](README.zh-TW.md) | [Français](README.fr.md) | [Polski](README.pl.md) | [Українська](README.uk.md) | [Dansk](README.da.md) | [العربية](README.ar.md)
...
> **🆕 最新版本 — v1.80.0**
>
> **五项扫描升级**（创意源自 [job-crawler](https://github.com/bracketouverte/job-crawler)，重新实现）：新增 **Teamtailor** 来源 —— 通过其公开的 `/jobs.rss` 订阅源接入按租户的 `<slug>.teamtailor.com` 站点（适配器共 **27 个**）；**来源隔离** —— 失效的 404/410 来源会被记录，并在后续扫描中跳过（14 天后自愈），消除反复出现的失效 slug 噪音；**Max per source** 上限字段（默认 ∞）；**Posted within** 时效筛选（24h / 7d / 30d）；以及 `#/scan` 上的**保存的搜索 + ★ 收藏**，持久化在 `localStorage` 中并带防御性校验。在 v1.79.0（We Work Remotely）、v1.78.x（国家筛选、自动刷新、Enter→扫描、可点击徽标）、v1.77.0（丹麦语）和 v1.76.0（按租户的 ATS 来源、`trust_filter`）的基础上构建。
>
> _13 locales · 6 个 LLM 提供方 · 27 个扫描器适配器 · 保存的搜索 + 收藏 · 与父级 career-ops v1.14.0 对齐。_
...
[career-ops](https://career-ops.org) 是一套开源求职系统,以 slash 命令的形式运行在任意 AI 编码 CLI(Claude Code、Gemini CLI、Codex、Qwen Code、OpenCode、GitHub Copilot CLI、Antigravity CLI — 其他 Claude 兼容 CLI 也通过相同的斜杠命令接口运行)中。模型无关。它基于六维 0.0–5.0 评分体系,将每个职位与你的 CV 匹配,生成定制 PDF 简历,并在本地跟踪每一次投递 —— 无云端账户、无遥测、无自动提交。
