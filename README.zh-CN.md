...
> **🆕 最新版本 — v1.84.0**
>
> **重复投递冷却期 + pipeline.md 薪资信息**（与上游 career-ops v1.15.0 对齐）：EN 扫描现在**跳过你近期已投递公司的职位**（`config/profile.yml → re_apply_windows:`，默认关闭），使结果聚焦于新招聘信息；扫描到的职位**薪资**作为可选的 `url | <comp>` 列持久化到 `data/pipeline.md`。在 v1.83.0（重复发布检测器）、v1.82.0（NoDesk）以及 v1.81.0（13 个新来源）的基础上构建。
>
> _13 locales · 6 个 LLM 提供方 · 41 个扫描器适配器 · 重复发布检测器 · 重复投递冷却期 · 与父级 career-ops v1.15.0 对齐。_
