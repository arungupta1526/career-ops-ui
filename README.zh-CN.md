...
> **🆕 最新版本 — v1.83.0**
>
> **重复发布 / 幽灵职位检测器**（与上游 career-ops v1.15.0 对齐）：`#/scan` 新增 **🔁 重复发布 / 幽灵职位** 面板，标记在 90 天滚动窗口内以不同 URL 重复发布的公司+职位集群 —— 过时招聘流水线 / 幽灵职位信号 —— 从您的扫描历史中读取（`GET /api/scan/reposts`）。在 v1.82.0（NoDesk）、v1.81.0（13 个新来源）以及 v1.80.0（Teamtailor、来源隔离、保存的搜索）的基础上构建。
>
> _13 locales · 6 个 LLM 提供方 · 41 个扫描器适配器 · 重复发布检测器 · 与父级 career-ops v1.15.0 对齐。_
