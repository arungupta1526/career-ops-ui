...
>
> **2. 各来源抓取超时从 15s → 30s**（可用 `SCAN_FETCH_TIMEOUT_MS` 覆盖）。Ashby 的 `includeCompensation` 负载在 ×8 并发下经常超过 15s，导致每次扫描约有 30 个 Ashby 看板超时；30s 让缓慢但存活的来源得以完成。
>
> _新增 13 项测试 · 完整套件 **1060/1060** 通过 · i18n 与文档已在全部 9 个语言同步。_
