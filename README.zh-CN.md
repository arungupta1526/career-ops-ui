...
> **v1.66.0 —— RU 来源现在遍历全部结果页。** hh.ru、Habr Career 与 Trudvsem 此前每次查询只取前 ~50 条；现在会翻到最后一页——hh.ru/Habr 用 `&page=N`，Trudvsem 用 `offset`/`meta.total`——跨页去重，并在某页没有新结果时（或到 50 页安全上限）停止。像「Backend разработчик」这样的查询现在返回完整结果而非一页（如 hh.ru PHP **17 → 55+** 跨 3 页；Trudvsem 返回全部 72 条）。逐页请求保留既有的超时 + `AbortSignal`。完整套件 **1045/1045** 通过。
