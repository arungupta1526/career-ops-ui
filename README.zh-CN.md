...
> **v1.67.0 —— `#/scan` 新增薪资区间筛选。** 结果表在文本与远程筛选旁新增两个数字输入（**薪资 下限 / 上限**）。每行的自由文本薪资（`от 100 000 до 200 000 ₽`、`120000-150000 USD`、`$120K–$150K` 等）会被解析为数字区间，并按区间重叠匹配；未标注薪资的行仍会显示，因此筛选只是缩小而非清空列表（不区分币种——不做汇率换算）。同时将各来源抓取超时从 **15s → 30s**，让缓慢的 Ashby 看板不再超时。完整套件 **1060/1060** 通过。
