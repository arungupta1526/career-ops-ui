...
> **🆕 最新版本 — v1.67.1**
>
> **1. `#/scan` 薪资区间筛选。** 结果表在文本与远程筛选旁新增两个数字输入（**薪资 下限 / 上限**）。每行的自由文本薪资（`от 100 000 до 200 000 ₽`、`120000-150000 USD`、`$120K–$150K` 等）会被解析为数字区间，并按区间重叠匹配——当某行的薪资区间与你的 `[下限, 上限]` 窗口重叠时保留。**未标注薪资的行仍会显示**，因此筛选只是缩小而非清空列表。比较**不区分币种**（不做汇率换算）。
>
> **2. 各来源抓取超时设为 10s（快速失败）。** v1.67.0 提到 30s 也只挽回约一半的缓慢 Ashby 看板——其余（Perplexity、Supabase、Resend、DeepL、Ramp 等）无论期限都会挂起，因此更长的超时只是让每次扫描在等待死槽时停滞。10s 对长期挂起者快速失败并保持扫描响应；可用 `SCAN_FETCH_TIMEOUT_MS` 覆盖。
>
> _完整套件 **1060/1060** 通过 · i18n 与文档已在全部 9 个语言同步。_
