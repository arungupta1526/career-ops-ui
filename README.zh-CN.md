...
> **🆕 最新版本 — v1.69.0**
>
> **扫描器来源插件自动发现 (P-14) — 将一个 `.mjs` 文件放入 `server/lib/sources/` 即可自动注册。** v1.69 之前，`server/lib/sources/registry.mjs` 中的来源列表需手工维护：添加一个 adapter 意味着同时编辑 `<id>.mjs` 和 `registry.mjs`。现在每个 adapter 只需声明一个自描述的 `export const meta = { value, label, region, configKey? }` 块，注册表会在启动时自动发现（`readdirSync` + 动态 `import()`）。关闭了路线图 P-14 的部分内容。公共 API（`SOURCES`、`SOURCES_BY_REGION`、`RU_CONFIG_KEYS`、`getRegionalSources`）保持不变 —— 所有现有导入继续有效，格式错误的 `meta` 会被跳过并产生一条诊断警告。帮助 §17（"如何添加新的招聘门户来源"）已在全部 9 个语言版本中按即插即用流程重写。
>
> _完整套件 **1079/1079** 通过 · i18n 与文档已在全部 9 个语言同步。_
