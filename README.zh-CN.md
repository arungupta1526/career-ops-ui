...
> **🆕 最新版本 — v1.81.0**
>
> **13 个新的求职板扫描来源**（与上游 career-ops 对齐）：全站公开 API **Arbeitnow · Himalayas · Jobicy · Landing.jobs · 4 Day Week · The Muse · The Hub · Jobspresso · Hacker News "Who is hiring?"**、波兰求职板 **JustJoin.it · NoFluffJobs**（POST 搜索），以及按租户的 ATS **Pinpoint · Rippling**（从 `careers_url` 自动识别）—— 全部以 `redirect:'error'` 锁定主机（防 SSRF），可在 `#/scan` 的 **Source** 下拉中选择（共 **40 个适配器**）。在 v1.80.0（Teamtailor、来源隔离、每来源上限、时效筛选、保存的搜索）、v1.79.0（We Work Remotely）、v1.77.0（丹麦语）和 v1.76.0（按租户的 ATS 来源）的基础上构建。
>
> _13 locales · 6 个 LLM 提供方 · 40 个扫描器适配器 · 13 个新来源 · 与父级 career-ops 对齐。_
