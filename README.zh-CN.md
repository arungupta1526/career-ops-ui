...
> **🆕 最新版本 — v1.79.0**
>
> **WeWorkRemotely 扫描来源（与父级 career-ops v1.14.0 对齐）。** 覆盖整个板块的 [We Work Remotely](https://weworkremotely.com) 远程职位 RSS 订阅源现已成为一等扫描来源 —— 添加一条 `provider: weworkremotely` 条目，它便会出现在 `#/scan` 的 **Source** 下拉中（共 **26 个适配器**）。主机固定 + `redirect:'error'`（SSRF 安全）；标题按 `Company: Role` 拆分。此外，`title_filter` 关键词会在长度检查前先行修剪（父级 #1261）。在 v1.78.x（扫描**国家筛选**、结果**自动刷新**、全局搜索 **Enter→扫描**、可点击徽标）、v1.77.0（丹麦语，第 13 个语言）和 v1.76.0（六个按租户的 ATS 来源、`trust_filter`、无上限的扫描）的基础上构建。
>
> _13 locales · 6 个 LLM 提供方 · 26 个扫描器适配器 · 国家筛选 · 与父级 career-ops v1.14.0 对齐。_
