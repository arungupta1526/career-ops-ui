...
> **🆕 最新版本 — v1.77.0**
>
> **丹麦语（Dansk）作为第 13 种界面语言加入。** 界面、应用内**帮助指南**（19 H2 / 75 H3）、README 和 CHANGELOG 的完整丹麦语翻译。丹麦语加入了国旗语言选择器，整套 i18n 机制（装配器、审计、对齐校验、快照）现在覆盖 **13 locales**。在 v1.76.0（与父级 career-ops v1.13.0 对齐 — 六个按租户的 ATS 来源 BambooHR / Breezy HR / Comeet / Personio / Recruitee / SolidJobs、`trust_filter`、无上限的扫描结果、标题过滤器健壮性）和 v1.70–74（阿拉伯语 RTL、6 个 LLM 提供方）的基础上构建。
>
> _13 locales · 6 个 LLM 提供方 · 25 个扫描器适配器 · 与父级 career-ops v1.13.0 对齐。_
