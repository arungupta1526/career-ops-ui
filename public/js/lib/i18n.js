...
window.I18n = (function () {
  ...
  const DICT = {
    ...
    'deep.tipManual': { en: 'Tip: set GEMINI_API_KEY in .env to run research live in the browser. Without it, the prompt is generated for you to paste into Claude Code.', es: 'Sugerencia: define GEMINI_API_KEY en .env para ejecutar la investigación en vivo. Sin ella se genera un prompt para pegarlo en Claude Code.', 'pt-BR': 'Dica: defina GEMINI_API_KEY em .env para rodar ao vivo. Sem ela, geramos o prompt para colar no Claude Code.', ko: '팁: .env 에 GEMINI_API_KEY 를 설정하면 브라우저에서 바로 실행됩니다. 없으면 Claude Code 에 붙여넣을 프롬프트만 생성합니다.', ja: 'ヒント: .env に GEMINI_API_KEY を設定するとブラウザ上で実行できます。未設定の場合は Claude Code に貼り付けるプロンプトのみ生成します。', ru: 'Подсказка: установите GEMINI_API_KEY в .env, чтобы запускать прямо в браузере. Без него генерируется только промпт для вставки в Claude Code.', 'zh-CN': '提示:在 .env 中设置 GEMINI_API_KEY 即可在浏览器中实时运行。否则仅生成可粘贴到 Claude Code 的 prompt。', 'zh-TW': '提示:在 .env 中設定 GEMINI_API_KEY 可在瀏覽器中即時執行。否則僅產生可貼到 Claude Code 的 prompt。' },
