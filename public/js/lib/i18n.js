...
window.I18n = (function () {
  ...
  const DICT = {
    ...
    'batch-prompt.deprecated': { en: 'This route is deprecated. The canonical Batch evaluate page is now at #/batch (TSV editor + parallel runner). This legacy prompt-builder will be removed in v1.16.0.', es: 'Esta ruta está obsoleta. La página canónica de Batch evaluate ahora está en #/batch (editor TSV + ejecutor paralelo). Este constructor de prompt legacy será eliminado en v1.16.0.', 'pt-BR': 'Esta rota está obsoleta. A página canônica de Batch evaluate agora está em #/batch (editor TSV + executor paralelo). Este construtor de prompt legacy será removido em v1.16.0.', ko: '이 경로는 더 이상 사용되지 않습니다. 표준 Batch evaluate 페이지는 이제 #/batch에 있습니다(TSV 편집기 + 병렬 실행기). 이 레거시 prompt-builder는 v1.16.0에서 제거됩니다.', ja: 'このルートは非推奨です。標準の Batch evaluate ページは #/batch にあります(TSV エディタ + 並列ランナー)。このレガシー prompt-builder は v1.16.0 で削除されます。', ru: 'Этот роут устарел. Канонический Batch evaluate теперь на #/batch (TSV редактор + параллельный раннер). Этот legacy prompt-builder будет удалён в v1.16.0.', 'zh-CN': '此路由已弃用。规范的 Batch evaluate 页面现在位于 #/batch (TSV 编辑器 + 并行运行器)。此旧版 prompt-builder 将在 v1.16.0 中移除。', 'zh-TW': '此路由已棄用。規範的 Batch evaluate 頁面現在位於 #/batch (TSV 編輯器 + 並行執行器)。此舊版 prompt-builder 將在 v1.16.0 中移除。' },
