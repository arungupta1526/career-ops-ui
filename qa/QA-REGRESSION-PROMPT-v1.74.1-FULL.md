# QA REGRESSION PROMPT — career-ops-ui v1.74.1 · FULL / EXHAUSTIVE (whole project)

> **Scope:** the *entire* career-ops-ui project, *all* functionality, as of `package.json` **1.74.1**. This is the single-pass, full-surface driver — it supersedes the v1.70.0 and v1.69.2 FULL prompts and folds in everything from v1.59 → v1.71.
...
> **Output:** save your run report to `qa/v55-regression/<YYYY-MM-DD>-REGRESSION-v1.74.1.md` with a PASS/FAIL per item and evidence (command output, HTTP traces, screenshots). One finding = one fix-ship (one-fix-per-release doctrine; HIGH → MEDIUM → LOW).
>
...
node scripts/check-changelog-parity.mjs     # "all 11 locales at v1.74.1"
...
- `parentVersion` reads the parent `VERSION` file (currently **1.11.0**); web-ui `version` from `package.json` (**1.74.1**) — they drift independently.
