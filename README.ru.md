...
[![tests](https://img.shields.io/badge/tests-1065%20passed-brightgreen)](#тесты)
...
> **🆕 Последний релиз — v1.68.2**
>
> **Команды CLI через `npx` / `npm link` были сломаны — исправлено.** npm и npx публикуют `career-ops-ui` как симлинк в `node_modules/.bin/`, и старая логика пути указывала на `.bin`, а не на корень пакета — поэтому `npx career-ops-ui init` падал с `MODULE_NOT_FOUND`. Теперь `bin/career-ops-ui.sh` и `bin/start.sh` резолвят путь к скрипту по цепочке симлинков (`readlink` + `cd -P`), так что любая команда работает из репозитория, через `npm link` и через `npx`. Закреплено новым регресс-тестом, прогоняющим команду через симлинк в стиле `.bin`.
>
> _Полный сьют **1065/1065** зелёный · i18n + документация синхронизированы во всех 9 локалях._
