...
[![tests](https://img.shields.io/badge/tests-1084%20passed-brightgreen)](#тесты)
...
> **🆕 Последний релиз — v1.69.1**
>
> **Два исправления.** **(1) scan:** таблица `#/scan` больше не обрезает молча крупные региональные сборы — отображаемый набор был ограничен 500 на регион (RU-скан из 1352 подходящих вакансий показывал только 500; 852 скрыты). Оба сканера теперь используют `MAX_STORED_RESULTS` (по умолчанию 2000, настраивается через `SCAN_MAX_RESULTS`); влияет только на отображение. **(2) health/ui:** карточки `#/health` больше не вылезают за пределы — длинное имя/значение сталкивалось с кнопкой Fix и бейджем; строка теперь сжимается и переносится через `.health-check-row`.
>
> _Полный сьют **1084/1084** зелёный · i18n + документация синхронизированы во всех 9 локалях._
