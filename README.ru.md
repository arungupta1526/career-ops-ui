...
[![tests](https://img.shields.io/badge/tests-1079%20passed-brightgreen)](#тесты)
...
> **🆕 Последний релиз — v1.69.0**
>
> **Авторегистрация сканера по принципу plug-in (P-14) — брось `.mjs` в `server/lib/sources/` и он зарегистрирует себя сам.** До v1.69 список источников в `server/lib/sources/registry.mjs` поддерживался вручную: добавление адаптера означало правку и `<id>.mjs`, и `registry.mjs`. Теперь каждый адаптер объявляет самоописывающий блок `export const meta = { value, label, region, configKey? }`, а реестр авто-обнаруживает его при старте (`readdirSync` + динамический `import()`). Закрывает частичную половину дорожного пункта P-14. Публичное API (`SOURCES`, `SOURCES_BY_REGION`, `RU_CONFIG_KEYS`, `getRegionalSources`) не изменилось — все существующие импорты продолжают работать, некорректный `meta` пропускается с единственным диагностическим предупреждением. Справка §17 («Как добавить новый источник для скана») переписана во всех 9 локалях под drop-in процесс.
>
> _Полный сьют **1079/1079** зелёный · i18n + документация синхронизированы во всех 9 локалях._
