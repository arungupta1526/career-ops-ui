...
> **v1.66.0 — РФ-источники теперь обходят все страницы результатов.** hh.ru, Habr Career и Trudvsem брали только первые ~50 вакансий на запрос; теперь идут по пагинации до конца — `&page=N` для hh.ru/Habr, `offset`/`meta.total` для Trudvsem — с дедупом между страницами и остановкой, когда страница не даёт ничего нового (или по предохранителю в 50 страниц). Запрос вроде «Backend разработчик» теперь возвращает весь результат, а не одну страницу (напр. hh.ru PHP **17 → 55+** за 3 страницы; Trudvsem отдаёт все 72). Постраничные запросы сохраняют существующий таймаут + `AbortSignal`. Полный сьют **1045/1045** зелёный.
