...
[![tests](https://img.shields.io/badge/tests-1063%20passed-brightgreen)](#тесты)
...
> **🆕 Последний релиз — v1.68.1**
>
> **1. Переработана панель фильтров на `#/scan`.** У каждого фильтра теперь подпись **сверху** (Поиск · Формат · Зарплата от / до · Источник · Объём), явные кнопки **«Применить»** + **«Сбросить»** и подсказка на странице. В «Формат» добавлен вариант **«В офисе»** (Remote / Hybrid / В офисе / Релокация).
>
> **2. Фильтр по зарплате наконец работает.** Задайте *от* / *до* — и в списке останутся только вакансии, чья оплата пересекается с вашим диапазоном; вакансии вне диапазона **и вакансии без указанной зарплаты** убираются (валюта не учитывается).
>
> **3. Таймаут запроса к источнику поднят до 60s** (переопределяется `SCAN_FETCH_TIMEOUT_MS`). 10s отбраковывал быстро, но обрезал и «медленные, но живые» доски; за минуту они успевают ответить. Компромисс: мёртвый источник держит слот всю минуту, а хронически висящие могут всё равно отваливаться.
>
> _Полный сьют **1063/1063** зелёный · i18n + документация синхронизированы во всех 9 локалях._
