...
[![tests](https://img.shields.io/badge/tests-1032%20passed-brightgreen)](#тесты)
...
- **hh.ru** — публичный API (с не-российских IP возвращает 403; блок идёт по **IP**, поэтому укажите `HH_PROXY` — российский IPv4-прокси, см. [Скан hh.ru из-за рубежа](#скан-hhru-из-за-рубежа-hh_proxy) — или пропустите источник: повторные 403 схлопываются, и он отключается на ходу).
...
hh.ru гео-блокирует свой публичный API по **IP-адресу**. С не-российского выходного узла каждый запрос к `api.hh.ru` возвращает **HTTP 403**; сканер логирует предупреждение, отключает hh.ru на этот прогон и продолжает с остальными источниками. Один `HH_USER_AGENT` блок **не снимает** — решает IP, а не User-Agent. Чтобы включить hh.ru из-за рубежа, направьте **только его запрос** через российский прокси переменной `HH_PROXY`; остальные источники сохраняют прямое соединение.

**1 — Возьмите российский прокси. Он должен быть IPv4, HTTP/HTTPS и residential.** Три независимых барьера, проверено на практике:
- **IPv4, не IPv6.** У `api.hh.ru` нет IPv6 (нет AAAA-записи). **IPv6-прокси до него не достучится** — на каждый запрос отдаёт `502 Bad Gateway / Host Not Found`. IPv6-прокси дешевле и идут по умолчанию, поэтому явно выбирайте **IPv4**.
- **HTTP/HTTPS, не SOCKS.** Прокси туннелирует HTTPS через `CONNECT`. **SOCKS5 в `HH_PROXY` не поддерживается** (реализация на `ProxyAgent` из undici).
- **Residential/мобильный, не датацентр.** hh.ru блокирует хостинговые/датацентровые диапазоны **403 `forbidden`** *даже если IP российский*. Прокси на Selectel/Timeweb/VDSina туннелирует нормально, но всё равно ловит 403. Нужен **российский residential или мобильный прокси** (реальный IP от ISP). Берите *резидентский* / *мобильный*, а не *серверный/датацентр* (proxy6.net, proxys.io, proxyline.net, ASocks и др.). **Оговорка:** перепродаваемые residential-*пулы* тоже часто детектятся — в тестах РФ-датацентр, РФ-residential *пул* (ER-Telecom, Барнаул) и полные браузерные заголовки **всё равно дали 403**. Надёжнее всего — **собственное домашнее российское подключение** сервера или OAuth-токен зарегистрированного приложения **dev.hh.ru**.
- Некоторые провайдеры дополнительно требуют **добавить исходящий IP вашего сервера в whitelist** в панели — иначе `CONNECT` падает с 502 даже при верном логине.

**2 — Проверьте прокси до подключения.** С сервера:
```bash
curl -x "http://ЛОГИН:ПАРОЛЬ@HOST:PORT" \
  "https://api.hh.ru/vacancies?text=php&per_page=1" -w "\nHTTP %{http_code}\n"
```
- `HTTP 200` + JSON → работает, продолжайте.
- `HTTP 403 {"type":"forbidden"}` → прокси достучался до hh.ru, но его IP заблокирован. Почти всегда это **датацентр** — берите residential/мобильный.
- `CONNECT tunnel failed, response 502` → прокси не доходит до hh.ru (IPv6-прокси, истёкший тариф или IP не в whitelist).
- `400 bad_user_agent` → hh.ru забанил User-Agent; оставьте штатный `HH_USER_AGENT` (строка вида `app/ver (email)`), а не мусорный.
- `407` → неверные креды прокси.

**3 — Пропишите.** Добавьте в `.env` в **корне проекта career-ops** (там же, где `HH_USER_AGENT`):
```
HH_PROXY=http://ЛОГИН:ПАРОЛЬ@HOST:PORT
```
Значение читается при старте — после правки **перезапустите сервер**. В `.env` лежит пароль — никогда не коммитьте его (файл в gitignore).

**4 — Проверьте.** Перезапустите скан на `#/scan`; результаты hh.ru появятся рядом с Habr Career. Всё ещё пропускается? Повторите `curl` из шага 2 — ломается прокси, а не приложение.
