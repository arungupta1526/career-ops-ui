...
> **🆕 Последний релиз — v1.82.0**
>
> **Источник сканирования NoDesk** (паритет с родительским career-ops v1.15.0): общедосочная RSS-лента удалённой работы NoDesk теперь полноценный источник — добавьте запись `provider: nodesk`, и она появится в выпадающем списке **Source** на `#/scan` (всего **41 адаптер**). Хост закреплён за `nodesk.co` с `redirect:'error'` (защита от SSRF); заголовки делятся по `Role at Company`. Основан на v1.81.0 (13 новых источников), v1.80.0 (Teamtailor, карантин источников, сохранённые поиски) и v1.77.0 (датский).
>
> _13 locales · 6 LLM-провайдеров · 41 адаптер сканера · паритет с родительским career-ops v1.15.0._
