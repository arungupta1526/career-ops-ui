...
> **v1.62.1 — RSS-адаптер: скан любого джоб-борда с RSS ([#15](https://github.com/Fighter90/career-ops-ui/pull/15)).** Новый адаптер `rss` (`server/lib/portals/adapters/rss.mjs` + `server/lib/sources/rss.mjs`) позволяет сканеру собирать вакансии из любого RSS/Atom-фида — **LaraJobs, WeWorkRemotely, RemoteOK, golangprojects** и других бордов вне Greenhouse/Ashby/Lever. Без новых зависимостей: парсинг фида на регулярках с поддержкой CDATA и HTML-сущностей (заголовки/компании очищаются от тегов, astral-символы декодируются безопасно). Включается для компании ключами `provider: rss` + `rss:` (или `feed_url:`) в `portals.yml` — без правок кода — и не перехватывает компании, уже сопоставленные с ATS. **RSS** теперь отображается в выпадающем фильтре источников на `#/scan` рядом с Greenhouse / Ashby / Lever / Workable / SmartRecruiters / Workday. `ALL_ADAPTERS` вырос 6 → 7. 29 новых тестов; полный сьют **1032/1032**. README и CHANGELOG обновлены во всех 9 локалях.
...
**Самый быстрый путь — один curl:**

```bash
curl -fsSL https://raw.githubusercontent.com/Fighter90/career-ops-ui/main/bin/setup.sh | bash
```

Команда клонирует оба репозитория (career-ops + career-ops-ui), устанавливает зависимости, запускает doctor и поднимает сервер по адресу http://127.0.0.1:4317.

**Или пошагово через CLI `career-ops-ui`** (clone → setup → init → run):

```bash
git clone https://github.com/Fighter90/career-ops-ui
cd career-ops-ui
npm link                 # открывает команду `career-ops-ui` (или: npx career-ops-ui <verb>)

career-ops-ui setup      # bootstrap: установка зависимостей → doctor → run (SKIP_START=1 — остановиться перед run)
career-ops-ui init       # интерактивно: выбор LLM-провайдера + вставка его ключа → запись в .env родительского проекта
career-ops-ui doctor     # проверка Node / проекта / ключей / Playwright (exit 0 ⇔ всё обязательное «зелёное»)
career-ops-ui run        # запуск сервера по адресу http://127.0.0.1:4317
career-ops-ui open       # открыть + ПОДНЯТЬ вкладку дашборда в браузере
career-ops-ui help       # список всех verb-ов
```

После `setup`/`run` вкладка дашборда открывается **и поднимается на передний план** автоматически; `career-ops-ui open` делает то же по требованию, так что не придётся искать вкладку. Задайте `NO_OPEN=1`, чтобы отключить авто-открытие (headless / CI).

`setup` сам прогоняет всю цепочку; остальные verb-ы также можно использовать по отдельности. `init` — это мастер выбора провайдера: выберите **Claude / Claude Code** (`ANTHROPIC_API_KEY`), **Gemini / Gemini CLI** (`GEMINI_API_KEY`), **Codex / OpenCode CLI** (`OPENAI_API_KEY`) или **Auto** (Anthropic → запасной Gemini). Ключи вводятся с отключённым эхо (ничего не остаётся в истории прокрутки shell) и записываются в родительский `career-ops/.env` через тот же проверенный путь, что использует вкладка API-ключей `#/config`. Неинтерактивная форма для CI:
...
Выбранный провайдер задаёт `LLM_PROVIDER` (`auto` | `claude` | `gemini`); живые маршруты сервера evaluate / deep-dive / mode / auto-pipeline учитывают его. Изменить его можно в любой момент из **`#/config` → API-ключи** без перезапуска.
