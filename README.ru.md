...
[![tests](https://img.shields.io/badge/tests-1238%20passed-brightgreen)](#тесты)
...
> **🆕 Последний релиз — v1.78.2**
>
> **Усиление i18n + UX.** Доступное имя логотипа бренда теперь локализовано на всех **13 языках** (`nav.logoHome`); нажатие **Enter** в глобальном поиске, когда вы уже на `#/scan`, принудительно перерисовывает страницу, так что предзаполненный запрос никогда не теряется; а `health.title` теперь переведён на польский (`Kondycja`) и датский (`Systemtilstand`). Основан на v1.78.1 (**автообновление** результатов сканирования + **Enter→Сканирование** в глобальном поиске + кликабельный логотип→главная), v1.78.0 (**гео-фильтр сканирования** с флагами), v1.77.0 (датский, 13-я локаль) и v1.76.0 (паритет с родительским career-ops v1.13.0 — шесть источников ATS на тенант, `trust_filter`, неограниченные результаты сканирования).
>
