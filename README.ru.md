...
> **🆕 Последний релиз — v1.84.0**
>
> **Кулдаун повторных заявок + компенсация в pipeline.md** (паритет с родительским career-ops v1.15.0): EN-скан теперь **пропускает роли в компаниях, в которые вы уже подавали заявку недавно** (`config/profile.yml → re_apply_windows:`, по умолчанию отключено), чтобы результаты оставались сфокусированы на новых вакансиях; найденные вакансии сохраняют **зарплату** в виде необязательного столбца `url | <comp>` в `data/pipeline.md`. Основан на v1.83.0 (детектор повторных публикаций), v1.82.0 (NoDesk) и v1.81.0 (13 новых источников).
>
> _13 locales · 6 LLM-провайдеров · 41 адаптер сканера · детектор повторных публикаций · кулдаун повторных заявок · паритет с родительским career-ops v1.15.0._
