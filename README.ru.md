...
[![tests](https://img.shields.io/badge/tests-1060%20passed-brightgreen)](#тесты)
...
> **🆕 Последний релиз — v1.67.1**
>
> **1. Фильтр по зарплате на `#/scan`.** В таблице результатов рядом с фильтрами по тексту и remote появились два числовых поля — **зарплата от / до**. Зарплата каждой строки в свободном тексте (`от 100 000 до 200 000 ₽`, `120000-150000 USD`, `$120K–$150K`, …) парсится в числовой диапазон и сопоставляется по пересечению диапазонов — строка остаётся, если её зарплатный диапазон пересекается с вашим окном `[от, до]`. Строки **без указанной зарплаты остаются видимыми**, поэтому фильтр сужает, а не обнуляет список. Сравнение **без учёта валюты** (без конвертации курса).
>
> **2. Таймаут запроса к источнику установлен в 10s (fail-fast).** Поднятие до 30s в v1.67.0 вернуло лишь ~половину медленных досок Ashby — остальные (Perplexity, Supabase, Resend, DeepL, Ramp, …) висят независимо от дедлайна, поэтому больший таймаут лишь тормозил весь скан, ожидая мёртвые слоты. 10s быстро отбраковывает хронически висящие источники и сохраняет отзывчивость скана; переопределяется `SCAN_FETCH_TIMEOUT_MS`.
>
> _Полный сьют **1060/1060** зелёный · i18n + документация синхронизированы во всех 9 локалях._
