...
> **🆕 Последний релиз — v1.78.1**
>
> **Улучшения UX сканирования.** Таблица результатов на `#/scan` теперь **автоматически обновляется** в реальном времени во время сканирования и ещё раз после его завершения — без ручной перезагрузки. Глобальный поиск в верхней панели показывает подсказку **Enter**, и при запросе, не являющемся URL, переходит на `#/scan` с **предзаполненным** полем поиска (раньше — `#/tracker`); **логотип бренда ведёт на дашборд** (главную). Основан на v1.78.0 (**гео-фильтр сканирования** — фильтрация результатов по стране с флагами, рядом с Remote / Hybrid / Onsite), v1.77.0 (датский, 13-я локаль) и v1.76.0 (паритет с родительским career-ops v1.13.0 — шесть источников ATS на тенант, `trust_filter`, неограниченные результаты сканирования).
>
