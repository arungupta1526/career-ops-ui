...
> **🆕 Последний релиз — v1.83.0**
>
> **Детектор повторных публикаций / фантомных вакансий** (паритет с родительским career-ops v1.15.0): на `#/scan` появилась панель **🔁 Повторные публикации / фантомные вакансии**, помечающая кластеры «компания + должность», повторно размещённые под разными URL в скользящем 90-дневном окне — сигнал устаревших воронок и фантомных объявлений — данные читаются из истории сканирования (`GET /api/scan/reposts`). Основан на v1.82.0 (NoDesk), v1.81.0 (13 новых источников) и v1.80.0 (Teamtailor, карантин источников, сохранённые поиски).
>
> _13 locales · 6 LLM-провайдеров · 41 адаптер сканера · детектор повторных публикаций · паритет с родительским career-ops v1.15.0._
