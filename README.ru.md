...
[![tests](https://img.shields.io/badge/tests-474%20passed-brightgreen)](#тесты)
[![playwright](https://img.shields.io/badge/playwright-32%20e2e-brightgreen)](#тесты)
...
[career-ops](https://career-ops.org) — это open-source-система поиска работы, которая работает как набор slash-команд внутри любого AI-CLI для программирования (Claude Code, Codex, Cursor, Gemini CLI, GitHub Copilot CLI). Система не привязана к конкретной модели. Она оценивает каждую вакансию относительно вашего CV по шестимерной шкале 0.0–5.0, генерирует адаптированные PDF-резюме и ведёт локальный учёт заявок — без облачных аккаунтов, телеметрии и автоматической отправки.
...
Команда клонирует оба репозитория (career-ops + career-ops-ui), устанавливает зависимости и поднимает сервер по адресу http://127.0.0.1:4317.
...
```bash
npm test                       # 474 unit/integration-теста
...
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **474** | Каждый эндпоинт, эфемерный сервер, без сети. Включая парсеры, сканер (mocked), runner, anthropic, заголовки безопасности, XSS, sanitize JD, валидацию URL, защиту от DNS-rebind, состояние гонки на трекере, rate-limit, path-traversal, паритет i18n. |
...
- Перед push выполняйте `npm test` — **474 проверки green** — это минимальная планка (плюс 32 Playwright, если изменения касаются UI).
