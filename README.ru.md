...
>
> **2. Таймаут запроса к источнику поднят с 15s → 30s** (переопределяется `SCAN_FETCH_TIMEOUT_MS`). Payload Ashby с `includeCompensation` под параллелизмом ×8 регулярно занимал более 15s, из-за чего ~30 досок Ashby отваливались по таймауту в каждом скане; 30s даёт медленным, но живым источникам завершиться.
>
> _13 новых тестов · полный сьют **1060/1060** зелёный · i18n + документация синхронизированы во всех 9 локалях._
