...
[![tests](https://img.shields.io/badge/tests-1244%20passed-brightgreen)](#тесты)
...
> **🆕 Последний релиз — v1.79.0**
>
> **Источник сканирования WeWorkRemotely (паритет с родительским career-ops v1.14.0).** Общедосочный RSS-фид удалённых вакансий [We Work Remotely](https://weworkremotely.com) теперь полноправный источник сканирования — добавьте запись `provider: weworkremotely`, и он появится в выпадающем списке **Source** на `#/scan` (всего **26 адаптеров**). Привязан к хосту + `redirect:'error'` (защита от SSRF); заголовки разбиваются по `Company: Role`. Также: ключевые слова `title_filter` теперь обрезаются перед проверкой длины (родительский #1261). Основан на v1.78.x (**фильтр по стране** при сканировании, **автообновление** результатов, **Enter→Сканирование** в глобальном поиске, кликабельный логотип), v1.77.0 (датский, 13-я локаль) и v1.76.0 (шесть источников ATS на тенант, `trust_filter`, неограниченное сканирование).
>
> _13 locales · 6 LLM-провайдеров · 26 адаптеров сканера · фильтр по стране · паритет с родительским career-ops v1.14.0._
