...
[![tests](https://img.shields.io/badge/tests-1258%20passed-brightgreen)](#тесты)
...
> **🆕 Последний релиз — v1.80.0**
>
> **Пять улучшений сканирования** (идеи из [job-crawler](https://github.com/bracketouverte/job-crawler), переосмысленные): источник **Teamtailor** — сайты `<slug>.teamtailor.com` на тенант через их публичный фид `/jobs.rss` (всего **27 адаптеров**); **карантин источников** — мёртвые источники с 404/410 фиксируются и пропускаются при последующих сканированиях (самовосстановление через 14 дней), что устраняет повторяющийся шум от мёртвых слагов; поле-ограничитель **Max per source** (∞ по умолчанию); фильтр по возрасту **Posted within** (24 ч / 7 д / 30 д); и **сохранённые поиски + ★ избранное** на `#/scan`, сохраняемые в `localStorage` с защитной валидацией. Основан на v1.79.0 (We Work Remotely), v1.78.x (фильтр по стране, автообновление, Enter→Сканирование, кликабельный логотип), v1.77.0 (датский) и v1.76.0 (источники ATS на тенант, `trust_filter`).
>
> _13 locales · 6 LLM-провайдеров · 27 адаптеров сканера · сохранённые поиски + избранное · паритет с родительским career-ops v1.14.0._
