...
> **🆕 最新版本 — v1.82.0**
>
> **NoDesk 掃描來源**（與父層 career-ops v1.15.0 對齊）：全站 NoDesk 遠端職缺 RSS 來源現已成為一級掃描來源——加入 `provider: nodesk` 條目後即出現在 `#/scan` 的 **Source** 下拉中（共 **41 個轉接器**）。主機鎖定為 `nodesk.co` 並使用 `redirect:'error'`（防 SSRF）；標題以 `Role at Company` 拆分。在 v1.81.0（13 個新來源）、v1.80.0（Teamtailor、來源隔離、已儲存搜尋）和 v1.77.0（丹麥語）的基礎上構建。
>
> _13 locales · 6 個 LLM 提供者 · 41 個掃描器轉接器 · 與父層 career-ops v1.15.0 對齊。_
