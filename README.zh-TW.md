...
> **🆕 最新版本 — v1.84.0**
>
> **重新申請冷卻 + pipeline.md 中的薪酬欄位**（與父層 career-ops v1.15.0 對齊）：EN 掃描現在**略過你近期已投遞過公司的職缺**（`config/profile.yml → re_apply_windows:`，預設關閉），讓結果聚焦於全新機會；掃描到的職缺同時以選填的 `url | <comp>` 欄位將**薪資**持久化至 `data/pipeline.md`。在 v1.83.0（重複刊登偵測器）、v1.82.0（NoDesk）和 v1.81.0（13 個新來源）的基礎上構建。
>
> _13 locales · 6 個 LLM 提供者 · 41 個掃描器轉接器 · 重複刊登偵測器 · 重新申請冷卻 · 與父層 career-ops v1.15.0 對齊。_
