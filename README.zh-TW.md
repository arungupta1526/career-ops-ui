...
> **🆕 最新版本 — v1.81.0**
>
> **13 個全新職缺看板掃描來源**（與父層 career-ops 對齊）：全站公開 API **Arbeitnow · Himalayas · Jobicy · Landing.jobs · 4 Day Week · The Muse · The Hub · Jobspresso · Hacker News "Who is hiring?"**、波蘭看板 **JustJoin.it · NoFluffJobs**（POST 搜尋），以及按租戶 ATS **Pinpoint · Rippling**（從 `careers_url` 自動辨識）—— 全部以 `redirect:'error'` 鎖定主機（防 SSRF），並可在 `#/scan` 的 **Source** 下拉中選取（共 **40 個轉接器**）。在 v1.80.0（Teamtailor、來源隔離、每來源上限、時效篩選、已儲存搜尋）、v1.79.0（We Work Remotely）、v1.77.0（丹麥語）和 v1.76.0（按租戶 ATS 來源）的基礎上構建。
>
> _13 locales · 6 個 LLM 提供者 · 40 個掃描器轉接器 · 13 個新來源 · 與父層 career-ops 對齊。_
