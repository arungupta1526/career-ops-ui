...
> **v1.67.0 —— `#/scan` 新增薪資區間篩選。** 結果表在文字與遠端篩選旁新增兩個數字輸入（**薪資 下限 / 上限**）。每列的自由文字薪資（`от 100 000 до 200 000 ₽`、`120000-150000 USD`、`$120K–$150K` 等）會被解析為數字區間，並依區間重疊比對；未標註薪資的列仍會顯示，因此篩選只是縮小而非清空清單（不分幣別——不做匯率換算）。同時將各來源抓取逾時從 **15s → 30s**，讓緩慢的 Ashby 看板不再逾時。完整套件 **1060/1060** 通過。
