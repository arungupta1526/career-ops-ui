...
- **觸發** 掃描(Greenhouse / Ashby / Lever / Workable / SmartRecruiters / Workday **以及** hh.ru / Habr Career),並即時觀看 SSE 日誌。
...
所有來源流經單一 SSE 端點:`/api/stream/scan?source=ats|regional|both`。UI 的 **🌐 Scan** 按鈕呼叫 `source=both`,因此所有 adapter(Greenhouse / Ashby / Lever / Workable / SmartRecruiters / Workday + hh.ru + Habr Career)會在同一條連線中執行。client 斷線時會 honor `AbortSignal` — 不會殘留 fetch。
...
```bash
npm test                       # 474+ 個 unit/integration 測試
npm run test:e2e               # 20 個 smoke e2e(自行啟動伺服器)
npm run test:e2e:full          # 23 個 comprehensive e2e
npm run test:e2e:browser       # 32 個 Playwright 瀏覽器 smoke
npm run test:coverage          # 同 `npm test`,加上 V8 coverage
```

| Suite                       | 測試數 | 內容                                                                                                         |
| --------------------------- | ----- | ------------------------------------------------------------------------------------------------------------ |
| `node --test tests/*.test.mjs`(unit + integration) | **474+** | 每個端點、暫時性伺服器、無網路。涵蓋解析器、scanner(已 mock)、runner、anthropic、CSP / 安全 header、實體感知 XSS、JD sanitize、URL 驗證、SSRF redirect rebind、檔案互斥下的並行 tracker 寫入、`llmRateLimit`、路徑統一 sanitization、i18n parity。 |
...
- 推送前執行 `npm test` — **474+ 項檢查綠燈**為門檻(若觸碰 UI 則加上 32 個 Playwright)。
