...
> **🆕 最新版本 — v1.83.0**
>
> **重複刊登 / 幽靈職缺偵測器**（與父層 career-ops v1.15.0 對齊）：`#/scan` 頁面新增 **🔁 重複刊登 / 幽靈職缺** 面板，標記同一公司＋職位組合在 90 天滾動視窗內以不同 URL 重複刊登的情形——此為招募管線停滯與幽靈職缺的信號——資料讀取自掃描紀錄（`GET /api/scan/reposts`）。在 v1.82.0（NoDesk）、v1.81.0（13 個新來源）和 v1.80.0（Teamtailor、來源隔離、已儲存搜尋）的基礎上構建。
>
> _13 locales · 6 個 LLM 提供者 · 41 個掃描器轉接器 · 重複刊登偵測器 · 與父層 career-ops v1.15.0 對齊。_
