...
> **🆕 最新版本 — v1.69.0**
>
> **插件式掃描器自動探索 (P-14) — 將 `.mjs` 放入 `server/lib/sources/` 即可自動註冊。** v1.69 之前,`server/lib/sources/registry.mjs` 中的來源清單需要手動維護:新增 adapter 意味著同時編輯 `<id>.mjs` 和 `registry.mjs`。現在每個 adapter 宣告一個自描述的 `export const meta = { value, label, region, configKey? }` 區塊,registry 在啟動時自動探索它(`readdirSync` + dynamic `import()`)。關閉了路線圖項目 P-14 的後半部分。公開 API(`SOURCES`、`SOURCES_BY_REGION`、`RU_CONFIG_KEYS`、`getRegionalSources`)保持不變 — 所有現有 import 繼續正常運作,格式錯誤的 `meta` 會被跳過並發出一條診斷警告。說明文件 §17(「如何新增職位入口網站來源」)已在全部 9 種語言中針對直接放入流程重新編寫。
>
> _完整套件 **1079/1079** 通過 · i18n 與文件已在全部 9 種語言同步。_
