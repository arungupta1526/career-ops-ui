...
> 用於 [career-ops](https://github.com/santifer/career-ops) AI 求職流水線的簡潔 docs-style Web 介面。
> 在單一瀏覽器分頁中搜尋、評估、深入研究、申請並追蹤每個職缺 — 不必再於 Claude Code、終端機與 markdown 檔案之間來回切換。
...
![career-ops-ui — 指揮中心](./images/dashboard-en.png)

## 關於 career-ops

[career-ops](https://career-ops.org) 是一套開源求職系統,以 slash 指令的形式運行於任何 AI 編碼 CLI(Claude Code、Codex、Cursor、Gemini CLI、GitHub Copilot CLI)內。模型無關。它以六維 0.0–5.0 評分準則對每個職缺與你的 CV 進行配對,產生量身打造的 PDF 履歷,並在本機追蹤每一次申請 — 無雲端帳號、無遙測、無自動提交。

**本儲存庫 (career-ops-ui)** 是建構於其上的精緻 Web 介面。CLI 仍負責 form-fill(透過 Playwright MCP)及 slash 指令模式;SPA 則在相同的 `cv.md` / `data/applications.md` / `reports/` 檔案之上,提供 CRM 風格的瀏覽器表面。兩者共用同一份資料。
...
| **4.0 – 4.4** | 送出申請,或 `/career-ops contacto` 取得 warm intro |
| **3.5 – 3.9** | `/career-ops deep` — 先做調研 |
| **< 3.5** | 除非另有理由,否則略過 |
...
此指令會 clone 兩個儲存庫(career-ops + career-ops-ui)、安裝相依套件,並在 http://127.0.0.1:4317 啟動伺服器。

---

## 為什麼?

[career-ops](https://github.com/santifer/career-ops) 是一套強大、由 Claude Code 驅動的求職系統:貼上 JD → 取得 0-5 適配分數、ATS 最佳化的 PDF 及追蹤器條目。它在 Claude Code 中運作良好,但資料散落於 `cv.md`、`data/applications.md`、`reports/*.md`、`data/pipeline.md`、`portals.yml`、`config/profile.yml` — 容易遺失,難以瀏覽。

`career-ops-ui` 在其上加上一層精緻 UI:

- **瀏覽** 追蹤器、報告與流水線,如同 CRM。
- **觸發** 掃描(Greenhouse / Ashby / Lever / Workable / SmartRecruiters / Workday **以及** hh.ru / Habr Career),並觀看即時 SSE 日誌。
- **評估** JD,優先透過 Anthropic(較佳)或 Gemini;若未設定 API 金鑰,則回傳給 Claude Code 的複製貼上 prompt。
- **深度研究** 公司,透過 Anthropic SDK 即時執行,並自動內嵌 cv / profile / mode 檔案。
- **編輯** `cv.md`,提供並排 markdown 預覽及伺服端 XSS 過濾。
- **維護** 系統:doctor、verify、normalize、dedup、merge — 每項一鍵完成。
- **多 CLI:** 可由 Claude Code、Codex、Cursor、Aider 或 Gemini CLI 等指令以相同方式驅動 — `CLAUDE.md` / `AGENTS.md` / `GEMINI.md` 的橋接檔指向單一事實來源。

純加法:`career-ops/` 內部不會被修改。你的所有自訂內容仍歸你所有。
...
依照 [career-ops onboarding](https://github.com/santifer/career-ops#first-run--onboarding) 設定,使 `cv.md`、`config/profile.yml`、`portals.yml` 已存在。

### 2. 將 career-ops-ui 放入其內
...
若你將儲存庫 clone 到其他位置(非 `career-ops/web-ui`),透過環境變數指向 career-ops:
...
| **career-ops** | 已 clone 且已完成 onboarding — 見上文 |
| **選用** | 父專案 `.env` 中的 `GEMINI_API_KEY`(免費層模型 `gemini-2.0-flash`)用於一鍵 JD 評估。否則 UI 會回傳給 Claude 的複製貼上 prompt。 |
| **選用** | 若 hh.ru 回傳 403,請從俄羅斯 IP / VPN 執行。Habr Career 則不受 IP 限制。 |
| **選用** | Playwright(已是 career-ops 的傳遞相依套件)用於執行 e2e 測試套件。 |
...
| **Scan**         | **🌐 單一 Scan 按鈕** — 一次執行所有已啟用的來源(EN:Greenhouse / Ashby / Lever / Workable / SmartRecruiters / Workday,RU:hh.ru + Habr Career)。即時 SSE 日誌串流 + 可點擊結果表,具備 location / Remote-Hybrid 徽章 / relocation 旗標 / salary / source 篩選器與動態 stack / level / keyword chip。Active-Companies 卡片列出每個受追蹤的 board 及其 API 健康狀態。 |
| **Pipeline**     | 對 `data/pipeline.md` 進行 CRUD。伺服端 preview proxy(SSRF-safe、逐 hop redirect 驗證、8 KB body cap)。可從 URL 直接跳到評估。 |
| **Evaluate**     | 貼上 JD → **Anthropic 優先**(兩個金鑰都有時較佳),其次 Gemini,再者為手動 prompt fallback。Anthropic 路徑會自動內嵌 cv / profile / `_shared.md` / `oferta.md`(REVIEW-A1)。可選擇將 JD 儲存至 `jds/`。 |
| **Deep research**| 與 Evaluate 採用相同的 fallback 鏈。即時 Anthropic 會回傳約 10-30 KB 經資料佐證的 markdown,並存至 `interview-prep/<company>-<role>.md`。 |
| **Modes**        | 7 個通用 mode 頁面(`/#/project`、`/#/training`、`/#/followup`、`/#/batch`、`/#/contacto`、`/#/interview-prep`、`/#/patterns`),具備相同的 Anthropic / Gemini / 手動 fallback。 |
| **Apply helper** | 產生送出申請的檢查清單;實際的 Playwright 表單填寫仍位於 Claude Code 中的 `/career-ops apply`。 |
| **Tracker**      | 對 `data/applications.md` 的可篩選表(狀態、分數、自由文字)。一鍵執行 `normalize-statuses.mjs` / `dedup-tracker.mjs` / `merge-tracker.mjs`。pipe 與換行的 escape 符合 GFM — 像 `"Acme \| Co"` 這樣的名稱可無損 round-trip。 |
| **Reports**      | 瀏覽並閱讀 `reports/` 下的每份報告,並解析其 header(Score / Legitimacy / URL)。                                      |
...
| **App settings** | UI 內的編輯器,可編輯父專案 `.env` 的金鑰:`ANTHROPIC_API_KEY`、`GEMINI_API_KEY`、模型 override、port / host。讀取時 secrets 會被遮罩。 |
| **Health**       | 所有 setup 檢查以 OK / OPTIONAL / FAIL 徽章呈現 + 執行 `doctor.mjs` 與 `verify-pipeline.mjs` 的按鈕。                  |
| **Help**         | 應用內 Markdown 使用指南(`/#/help`),已本地化為所有 8 種支援語言(en / es / pt-BR / ko-KR / ja / ru / zh-CN / zh-TW)。 |
| **Activity log** | 每筆會變更狀態的請求(寫入、執行、掃描)的稽核軌跡。secrets 會被遮蔽。 |

全域鍵盤快捷鍵:

- `Ctrl+K` / `Cmd+K` — 聚焦全域搜尋。
- 將 URL 貼到全域搜尋會自動將其加入 pipeline。
- `Esc` — 關閉任何開啟的 modal。
...
零 token 的入口網站掃描,實際會回傳職缺。UI 中的 **單一 🌐 Scan 按鈕** 會一次掃過所有已設定的來源:

- **Greenhouse / Ashby / Lever / Workable / SmartRecruiters / Workday** — 對 `portals.yml::tracked_companies` 中所有具備可辨識 ATS 模式的公司呼叫公開 boards-api。內建清單涵蓋 Stripe、GitLab、Vercel、Cloudflare、Datadog、Discord、Elastic、Grafana Labs、CockroachDB、Fastly、Twilio、Coinbase、Reddit、Robinhood、Affirm、Lyft、Linear、Supabase、PostHog、Ramp、Modal Labs、Railway、Browserbase、JetBrains — 可自由擴充或精簡。
- **hh.ru** — 公開 API(非 RU IP 會回傳 403;請從俄羅斯 IP / VPN 執行,或略過 — 來自同一來源的重複 403 會被合併,且該來源會在 run 中途被停用)。伺服器內建合理的預設 User-Agent;進階使用者仍可透過俄羅斯 IP / VPN override。
...
所有來源流經單一 SSE 端點:`/api/stream/scan?source=ats|regional|both`。UI 的 **🌐 Scan** 按鈕呼叫 `source=both`,因此所有 adapter(Greenhouse / Ashby / Lever / Workable / SmartRecruiters / Workday + hh.ru + Habr Career)會在同一條連線中執行。client 斷線時會 honor `AbortSignal` — 沒有殘留 fetch。
...
│     ├─ parsers.mjs         # markdown / pipeline / report parser(符合 GFM 的 pipe escape)
...
├─ public/                   # 靜態 SPA — 無建置步驟
│  ├─ index.html
│  ├─ css/app.css            # 設計 token(docs-style 調色盤)
│  └─ js/
...
└─ tests/                    # 284 unit + 12 Playwright + 23 e2e:full + 20 e2e:smoke
   ├─ parsers.test.mjs       # markdown / pipeline / report parser(純函式)
   ...
   ├─ playwright-smoke.mjs   # 12 個瀏覽器流程(CV 儲存、tracker、pipeline、evaluate、config 等)
...
| GET    | `/api/help/:lang`        | 本地化的應用內使用指南(fallback:`en.md`)                                  |
...
| POST   | `/api/tracker`                      | body `{ company, role, score?, status?, url?, notes?, date? }` — 具備 dedup(company + role 大小寫不敏感) |
| GET    | `/api/pipeline`                     | `{ urls: [...] }`                                                   |
| POST   | `/api/pipeline`                     | body `{ url }` → 透過 dedup + `isValidJobUrl` 加入 `data/pipeline.md` |
| GET    | `/api/pipeline/preview?url=…`       | 伺服端 fetch proxy(逐 hop SSRF 檢查、≤3 redirects、8 KB cap)       |
...
| DELETE | `/api/jds/:name`                    | unlink(需 `.txt` 副檔名)                                            |
| GET    | `/api/cv`                           | `{ markdown }`                                                       |
| PUT    | `/api/cv`                           | body `{ markdown }` → 寫入 `cv.md`(已 XSS 過濾、≤1 MB)              |
...
| Method | Path                                | 目的                                                                              |
| ------ | ----------------------------------- | --------------------------------------------------------------------------------- |
| POST   | `/api/evaluate`                     | body `{ jd, save? }` → JD 評估(依 `oferta.md` 的 A–G 區段)                       |
...
| POST   | `/api/deep`                         | body `{ company, role?, run? }` → 深度研究 prompt 或即時 grounded markdown          |
| POST   | `/api/mode/:slug`                   | 通用 mode runner;allowlist:`batch`、`contacto`、`followup`、`interview-prep`、`patterns`、`project`、`training` |
...
當 `/api/deep` 或 `/api/mode/:slug` 設定 `run: true` 時,伺服器會優先採用 Anthropic(兩個金鑰皆有時),將 `cv.md` + `config/profile.yml` + `modes/_shared.md` + 相關 mode 模板 內嵌至 `<project_context>` 區塊,並直接回傳模型的 grounded markdown。軟上限:組裝後的 prompt 200 KB — 超出會回傳 413。
...
```bash
npm test                       # 284 個 unit/integration 測試
npm run test:e2e               # 20 個 smoke e2e(自行 boot 伺服器)
npm run test:e2e:full          # 23 個 comprehensive e2e
npm run test:e2e:browser       # 12 個 Playwright 瀏覽器 smoke
npm run test:coverage          # 同 `npm test`,加上 V8 coverage
```

| Suite                       | 測試數 | 內容                                                                                                         |
| --------------------------- | ----- | ------------------------------------------------------------------------------------------------------------ |
| `node --test tests/*.test.mjs`(unit + integration) | **284** | 每個端點、暫時性伺服器、無網路。涵蓋 parser、scanner(已 mock)、runner、anthropic、security headers、XSS、JD sanitize、URL 驗證、i18n parity。 |
...
| `tests/playwright-smoke.mjs`(`npm run test:e2e:browser`) | **12** | 瀏覽器驅動 smoke:dashboard render、navigation、語言切換、404、health、tracker round-trip(BF-1)、pipeline add + 無效 URL 掃描、reports empty、evaluate 手動 fallback、config 金鑰遮罩、CV PUT XSS strip、pipeline preview 400。 |
| **總計**                   | **339** | **0 fails、0 flakes**                                                                                        |

Coverage:透過 `--experimental-test-coverage` ~93% 列 / ~83% 分支。

Parsers 為純函式(無 I/O)— 對來自 `applications.md`、`pipeline.md`、`reports/*.md` 的真實資料片段進行測試。API 測試在暫時連接埠上 boot Express app,並對每個端點進行端到端測試。Scanner 測試 mock `fetch`,因此即使 hh.ru 封鎖你的 IP 仍會通過。Playwright 瀏覽器 smoke 對 in-process 伺服器執行,並透過父專案的 `node_modules` 解析 Playwright — `web-ui/` 中沒有新相依套件。
...
| `HOST`               | `127.0.0.1`        | Express 綁定的 host。非 loopback 時會附加 CSP;auth gate 計畫於 v2.0.0。                |
| `CAREER_OPS_ROOT`    | 由 script 取 `..`  | `cv.md`、`data/`、`portals.yml`、`modes/` 等檔案的所在位置。                            |
| `ANTHROPIC_API_KEY`  | unset              | 啟用 `/api/evaluate`、`/api/deep`、`/api/mode/:slug` 的 live 模式(兩個金鑰皆有時較佳)。 |
...
| `(server uses default UA)`      | unset              | Override hh.ru User-Agent(有助減少非 RU IP 的 403)                       |
...
你也可以對任何公司條目擴充明確的 `api:` URL。參見本儲存庫的 [`docs/portals-examples.md`](docs/portals-examples.md),內有 24 間經驗證公司可直接貼上的區塊。
...
- 所有來自 client 的檔案路徑輸入皆會被 sanitize(`replace(/[^\w\-.]/g, '')`)。
- Subprocess 呼叫使用 `spawn` 配合參數陣列 — **絕無 shell interpolation**。
- 串流端點在 client 斷線時會 kill 子處理程序(無孤兒 scanner)。
- 寫入端點只觸碰已知的 career-ops 路徑:`data/`、`jds/`、`cv.md`、`config/`、`portals.yml`、`output/`。其他位置一概不碰。
- 連線橫幅在斷線時每 3 秒 ping `/api/health`,並在恢復時自動清除 — 不會 toast 轟炸。
...
1. **Anthropic(較佳)** — 在父專案 `.env` 中設定 `ANTHROPIC_API_KEY`。透過 `runAnthropic` 路由,並自動內嵌 `cv.md` / `config/profile.yml` / `modes/_shared.md` / mode 模板(REVIEW-A1)。已於 v1.8.0+ 驗證 live 運作,使用 `claude-sonnet-4-6` 為一次深度研究呼叫回傳 26 KB 經佐證的 markdown。
...
Claude Code 中既有的 `/career-ops apply` Playwright form-fill 流程仍是唯一能真正自動填寫申請表單的方法 — UI 的 *Apply helper* 僅產生檢查清單。
...
- 推送前執行 `npm test` — **284 項檢查綠燈**為門檻(若觸碰 UI 則加上 12 個 Playwright)。
- 非微不足道的變更走 GSD pipeline。見 [`docs/sdd/SDD-GUIDE.md`](docs/sdd/SDD-GUIDE.md)。
...
從非 Claude CLI(Codex、Aider、Cursor、Gemini)驅動本儲存庫?請閱讀 [`AGENTS.md`](AGENTS.md) 或 [`GEMINI.md`](GEMINI.md) — 兩者都橋接至規範的 `CLAUDE.md`。
...
一鍵安裝後,你有兩個空的 git clone,並已腳手架式地建立 `cv.md`、`config/profile.yml`、`portals.yml`、`data/applications.md`、`data/pipeline.md` 等檔案,其中包含 **EDIT ME** 標記。首次啟動時 Health 頁面應已全部綠燈。請以你的真實資料取代預留位置:
...
- **選項 B — 從 UI 上傳:** 在側欄點擊 **CV** → **📁 Upload CV** → 選擇你的 `.md` / `.txt` 檔 → 檢視預覽 →
  點擊 **💾 Save**。
- **選項 C — 將你的 LinkedIn URL 交給 Claude Code:** 在 `career-ops/` 中開啟 Claude Code,
...
替換姓名全稱、電子郵件、所在地、LinkedIn、目標角色、archetypes、薪資目標的預留位置。**archetypes** 是最重要的欄位 — 它們是每個 JD 對你進行配對的依據。
...
將 `title_filter.positive`(例如 `"PHP"`、`"Go"`、`"Backend"`、`"Senior"`)與 `title_filter.negative`(例如 `"Junior"`、`"Java"`、`"iOS"`)設定為你的技術棧與資歷層級。內建的 `tracked_companies` 清單已包含 3 個經驗證的 Greenhouse / Ashby board(GitLab、Vercel、Linear)。如需 24+ 個可直接貼上的區塊,請見 [`docs/portals-examples.md`](docs/portals-examples.md)。

若你想要 hh.ru / Habr Career 掃描,請編輯 setup script 建立的 `russian_portals:` 區塊 — 加入你的搜尋 query(例如 `"Senior PHP"`、`"Тимлид Go"`)。

### 4. (選用)LLM API 金鑰

當兩者皆存在時,UI 會優先採用 Anthropic 而非 Gemini。任何一個或都沒有都可以 — 無金鑰時,**Evaluate** 會回傳給 Claude Code 的複製貼上 prompt。
