...
> **🆕 最新版本 — v1.68.2**
>
> **透過 `npx` / `npm link` 呼叫的 CLI 動詞先前已損壞——現已修復。** npm 與 npx 會將 `career-ops-ui` 公開為 `node_modules/.bin/` 下的符號連結，而舊的路徑邏輯指向的是 `.bin` 而非套件根目錄，於是 `npx career-ops-ui init` 會以 `MODULE_NOT_FOUND` 崩潰。現在 `bin/career-ops-ui.sh` 與 `bin/start.sh` 會沿符號連結鏈解析腳本路徑（`readlink` + `cd -P`），因此每個動詞都能從儲存庫、透過 `npm link` 以及透過 `npx` 正常運作。已由一個透過 `.bin` 風格符號連結執行動詞的新回歸測試鎖定。
>
> _完整套件 **1065/1065** 通過 · i18n 與文件已在全部 9 種語言同步。_
