...
[English](README.md) | [Español](README.es.md) | [Português (Brasil)](README.pt-BR.md) | [한국어](README.ko-KR.md) | [日本語](README.ja.md) | [Русский](README.ru.md) | [简体中文](README.zh-CN.md) | **繁體中文** | [Français](README.fr.md) | [Polski](README.pl.md) | [Українська](README.uk.md) | [Dansk](README.da.md) | [العربية](README.ar.md)
...
> **🆕 最新版本 — v1.80.0**
>
> **五項掃描升級**（靈感來自 [job-crawler](https://github.com/bracketouverte/job-crawler),重新實作）:**Teamtailor** 來源 —— 透過其公開的 `/jobs.rss` 訂閱源支援按租戶的 `<slug>.teamtailor.com` 站點(總計 **27 個轉接器**);**來源隔離** —— 失效的 404/410 來源會被記錄並在後續掃描中略過(14 天後自我修復),消除反覆出現的失效 slug 雜訊;一個 **Max per source** 上限欄位(預設 ∞);一個 **Posted within** 時效篩選(24h / 7d / 30d);以及 `#/scan` 上的 **saved searches + ★ favorites**,持久化於 `localStorage` 並帶有防禦性驗證。在 v1.79.0(We Work Remotely)、v1.78.x(國家篩選、自動重新整理、Enter→掃描、可點擊標誌)、v1.77.0(丹麥語)和 v1.76.0(按租戶的 ATS 來源、`trust_filter`)的基礎上構建。
>
> _13 locales · 6 個 LLM 提供者 · 27 個掃描器轉接器 · saved searches + favorites · 與父層 career-ops v1.14.0 對齊。_
...
[career-ops](https://career-ops.org) 是一套開源求職系統,以 slash 指令的形式運行於任何 AI 編碼 CLI(Claude Code、Gemini CLI、Codex、Qwen Code、OpenCode、GitHub Copilot CLI、Antigravity CLI — 其他 Claude 相容 CLI 也透過相同的斜線指令介面運作)之內。模型無關。它以六維度 0.0–5.0 評分準則將每一筆職缺與你的 CV 進行配對、產生量身打造的 PDF 履歷,並在本機追蹤每一次投遞 — 不需要雲端帳號、不收集遙測資料、絕不自動送出。
