...
> **🆕 最新版本 — v1.69.1**
>
> **兩處修復。** **(1) scan：** `#/scan` 表格不再靜默截斷大型區域掃描 —— 顯示集被硬限制為每區域 500 筆（1352 筆相符的 RU 掃描只顯示 500 筆，隱藏 852 筆）。兩個掃描器現在使用 `MAX_STORED_RESULTS`（預設 2000，可經 `SCAN_MAX_RESULTS` 設定），僅影響顯示。**(2) health/ui：** `#/health` 檢查卡片不再溢位 —— 過長的名稱/值會與 Fix 按鈕和徽章重疊；該列現在透過 `.health-check-row` 收縮並換行。
>
> _完整套件 **1084/1084** 通過 · i18n 與文件已在全部 9 種語言同步。_
