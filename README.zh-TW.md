...
> **🆕 最新版本 — v1.77.0**
>
> **丹麥語（Dansk）作為第 13 種介面語言加入。** 介面、應用內**說明指南**（19 H2 / 75 H3）、README 和 CHANGELOG 的完整丹麥語翻譯。丹麥語加入了國旗語言選擇器，整套 i18n 機制（組裝器、稽核、對齊驗證、快照）現在涵蓋 **13 locales**。在 v1.76.0（與父層 career-ops v1.13.0 對齊 — 六個按租戶的 ATS 來源 BambooHR / Breezy HR / Comeet / Personio / Recruitee / SolidJobs、`trust_filter`、無上限的掃描結果、標題過濾器健壯性）和 v1.70–74（阿拉伯語 RTL、6 個 LLM 提供者）的基礎上構建。
>
> _13 locales · 6 個 LLM 提供者 · 25 個掃描器轉接器 · 與父層 career-ops v1.13.0 對齊。_
