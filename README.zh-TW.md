...
>
> _完整測試套件 **1126/1126** 通過 · 12 locales · 文件已同步。_
...
[career-ops](https://career-ops.org) 是一套開源求職系統,以 slash 指令的形式運行於任何 AI 編碼 CLI(Claude Code、Gemini CLI、Codex、Qwen Code、OpenCode、GitHub Copilot CLI — 其他 Claude 相容 CLI 也透過相同的斜線指令介面運作)之內。模型無關。它以六維度 0.0–5.0 評分準則將每一筆職缺與你的 CV 進行配對、產生量身打造的 PDF 履歷,並在本機追蹤每一次投遞 — 不需要雲端帳號、不收集遙測資料、絕不自動送出。
