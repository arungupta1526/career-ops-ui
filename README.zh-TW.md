...
>
> **2. 各來源抓取逾時從 15s → 30s**（可用 `SCAN_FETCH_TIMEOUT_MS` 覆寫）。Ashby 的 `includeCompensation` 負載在 ×8 並發下經常超過 15s，導致每次掃描約有 30 個 Ashby 看板逾時；30s 讓緩慢但存活的來源得以完成。
>
> _新增 13 項測試 · 完整套件 **1060/1060** 通過 · i18n 與文件已在全部 9 種語言同步。_
