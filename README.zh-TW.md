...
> **🆕 最新版本 — v1.67.1**
>
> **1. `#/scan` 薪資區間篩選。** 結果表在文字與遠端篩選旁新增兩個數字輸入（**薪資 下限 / 上限**）。每列的自由文字薪資（`от 100 000 до 200 000 ₽`、`120000-150000 USD`、`$120K–$150K` 等）會被解析為數字區間，並依區間重疊比對——當某列的薪資區間與你的 `[下限, 上限]` 視窗重疊時保留。**未標註薪資的列仍會顯示**，因此篩選只是縮小而非清空清單。比較**不分幣別**（不做匯率換算）。
>
> **2. 各來源抓取逾時設為 10s（快速失敗）。** v1.67.0 提到 30s 也只挽回約一半的緩慢 Ashby 看板——其餘（Perplexity、Supabase、Resend、DeepL、Ramp 等）無論期限都會掛起，因此更長的逾時只是讓每次掃描在等待死槽時停滯。10s 對長期掛起者快速失敗並保持掃描回應；可用 `SCAN_FETCH_TIMEOUT_MS` 覆寫。
>
> _完整套件 **1060/1060** 通過 · i18n 與文件已在全部 9 種語言同步。_
