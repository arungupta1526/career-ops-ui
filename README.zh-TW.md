...
> **v1.62.1 — RSS 轉接器:掃描任意基於 RSS 的徵才看板 ([#15](https://github.com/Fighter90/career-ops-ui/pull/15)).** 新增 `rss` 轉接器(`server/lib/portals/adapters/rss.mjs` + `server/lib/sources/rss.mjs`),讓掃描器能從任意 RSS/Atom 來源擷取職缺 —— **LaraJobs、WeWorkRemotely、RemoteOK、golangprojects** 以及 Greenhouse/Ashby/Lever 以外的其他看板。無新增相依套件:以正規表示式解析來源,支援 CDATA 與 HTML 實體解碼(標題/公司名去除標籤,星位碼點安全解碼)。透過 `portals.yml` 的 `provider: rss` + `rss:`(或 `feed_url:`)鍵依公司啟用 —— 無需改動程式碼 —— 且不會攔截已比對到 ATS 的公司。**RSS** 現已顯示在 `#/scan` 的來源篩選下拉選單中,與 Greenhouse / Ashby / Lever / Workable / SmartRecruiters / Workday 並列。`ALL_ADAPTERS` 由 6 增至 7。新增 29 項測試,完整套件 **1032/1032**。全部 9 個語系的 README 與 CHANGELOG 皆已更新。
