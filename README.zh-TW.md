...
> **近期 (v1.55 → v1.56):** 螢上 4 提供方引導橫幅 + 作用中提供方徽章、誠實的 ⚡ 即時執行費用提示、`#/auto` ETA、`#/scan` 進階篩選摺疊 + 醒目 Stop、`#/dashboard` hero CTA、`#/pipeline` 超 1000 列虛擬化、`#/tracker` 伺服器端分頁 + 漏斗晶片、無障礙打磨 —— 每版本細節見 [CHANGELOG.zh-TW.md](CHANGELOG.zh-TW.md)。
...
此指令會 clone 兩個儲存庫(career-ops + career-ops-ui)、安裝相依套件、執行 doctor,並在 http://127.0.0.1:4317 啟動伺服器。

**或使用 `career-ops-ui` CLI 逐步進行**(clone → setup → init → run):

```bash
git clone https://github.com/Fighter90/career-ops-ui
cd career-ops-ui
npm link                 # 公開 `career-ops-ui` 指令(或:npx career-ops-ui <verb>)

career-ops-ui setup      # 啟動程序:安裝相依套件 → doctor → run(SKIP_START=1 可在 run 前停止)
...
你所選的供應商會設定 `LLM_PROVIDER`(`auto` | `claude` | `gemini`);伺服器的即時 evaluate / deep-dive / mode / auto-pipeline 路由皆會遵循它。你隨時可從 **`#/config` → API 金鑰** 變更,無須重新啟動。
