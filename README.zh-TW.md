...
> **🆕 最新版本 — v1.79.0**
>
> **WeWorkRemotely 掃描來源（與父層 career-ops v1.14.0 對齊）。** 全板塊的 [We Work Remotely](https://weworkremotely.com) 遠端職缺 RSS 訂閱源現已成為一級掃描來源 —— 新增一條 `provider: weworkremotely` 條目,它便會出現在 `#/scan` 的 **Source** 下拉選單中(共 **26 個轉接器**)。已綁定主機 + `redirect:'error'`(防 SSRF);標題以 `Company: Role` 分割。此外:`title_filter` 關鍵字現會在長度檢查前先修剪空白(父層 #1261)。在 v1.78.x(掃描**國家篩選**、結果**自動重新整理**、全域搜尋 **Enter→掃描**、可點擊標誌)、v1.77.0(丹麥語,第 13 個語言)和 v1.76.0(六個按租戶的 ATS 來源、`trust_filter`、無上限的掃描)的基礎上構建。
>
> _13 locales · 6 個 LLM 提供者 · 26 個掃描器轉接器 · 國家篩選 · 與父層 career-ops v1.14.0 對齊。_
