...
[![tests](https://img.shields.io/badge/tests-474%2B%20passed-brightgreen)](#tests)
...
이 명령은 두 저장소(career-ops + career-ops-ui)를 클론하고, 의존성을 설치한 뒤, http://127.0.0.1:4317 에서 서버를 시작합니다.
...
│  └─ reviews/REVIEW-*.md
└─ tests/                    # 442 unit + 32 Playwright + 23 e2e:full + 20 e2e:smoke
...
```bash
npm test                       # 442 unit/integration 테스트
...
| Suite                       | Tests | 내용                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **442** | 모든 엔드포인트, 임시 서버, 네트워크 없음. parser, scanner (mocked), runner, anthropic, security headers, XSS, JD sanitize, URL validation, path traversal, DNS-rebind, file lock, rate limit, i18n 패리티 포함. |
...
- 푸시 전에 `npm test`를 실행합니다 — **474+ checks green**이 기준입니다(UI를 건드리는 경우 32개의 Playwright 테스트도 포함).
