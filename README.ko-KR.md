...
> **🆕 최신 릴리스 — v1.81.0**
>
> **13개의 새로운 채용 보드 스캔 소스**(상위 career-ops 패리티): 보드 전체 공개 API **Arbeitnow · Himalayas · Jobicy · Landing.jobs · 4 Day Week · The Muse · The Hub · Jobspresso · Hacker News "Who is hiring?"**, 폴란드 보드 **JustJoin.it · NoFluffJobs**(POST 검색), 테넌트별 ATS **Pinpoint · Rippling**(`careers_url`에서 자동 감지) — 모두 `redirect:'error'`로 호스트 고정(SSRF 방지)되며 `#/scan` **Source** 드롭다운에서 선택 가능합니다(**40개 어댑터** 총). v1.80.0(Teamtailor, 소스 격리, 소스당 최대, 기간 필터, 저장된 검색), v1.79.0(We Work Remotely), v1.77.0(덴마크어), v1.76.0(테넌트별 ATS 소스)을 기반으로 합니다.
>
> _13 locales · 6개 LLM 프로바이더 · 40개 스캐너 어댑터 · 13개 신규 소스 · 상위 career-ops 패리티._
