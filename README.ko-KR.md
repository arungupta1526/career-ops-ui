...
> **🆕 최신 릴리스 — v1.69.1**
>
> **두 가지 수정.** **(1) scan:** `#/scan` 테이블이 대규모 지역 스윕을 조용히 잘라내지 않습니다 — 표시 집합이 리전별 500개로 고정되어 있었습니다(일치 1352개의 RU 스캔이 500개만 표시, 852개 숨김). 두 스캐너 모두 이제 `MAX_STORED_RESULTS`(기본 2000, `SCAN_MAX_RESULTS`로 설정)를 사용하며, 표시에만 영향을 줍니다. **(2) health/ui:** `#/health` 점검 카드가 더 이상 넘치지 않습니다 — 긴 이름/값이 Fix 버튼 및 배지와 충돌했으나, 이제 `.health-check-row`로 축소·줄바꿈됩니다.
>
> _전체 스위트 **1084/1084** 통과 · i18n + 문서를 9개 로케일에 동기화._
