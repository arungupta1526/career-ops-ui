...
> **🆕 최신 릴리스 — v1.69.0**
>
> **플러그인 스캐너 자동 검색 (P-14) — `server/lib/sources/`에 `.mjs`를 드롭하면 자동 등록됩니다.** v1.69 이전에는 `server/lib/sources/registry.mjs`의 소스 목록을 직접 관리했습니다: 어댑터를 추가하려면 `<id>.mjs`와 `registry.mjs` 두 파일을 모두 편집해야 했습니다. 이제 모든 어댑터가 자기 설명적인 `export const meta = { value, label, region, configKey? }` 블록을 선언하면, 레지스트리가 부팅 시 자동으로 검색합니다(`readdirSync` + 동적 `import()`). 로드맵 항목 P-14의 절반을 닫습니다. 공개 API(`SOURCES`, `SOURCES_BY_REGION`, `RU_CONFIG_KEYS`, `getRegionalSources`)는 변경되지 않습니다 — 기존 임포트는 모두 그대로 동작하며, 잘못된 `meta`는 진단 경고 하나와 함께 건너뜁니다. 도움말 §17("새 채용 포털 소스를 추가하는 방법")을 9개 로케일 모두에서 드롭인 흐름으로 다시 작성했습니다.
>
> _전체 스위트 **1079/1079** 통과 · i18n + 문서를 9개 로케일에 동기화._
