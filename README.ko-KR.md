...
> **🆕 최신 릴리스 — v1.68.2**
>
> **`npx` / `npm link`로 실행하는 CLI 동사가 깨져 있었음 — 이제 수정됨.** npm과 npx는 `career-ops-ui`를 `node_modules/.bin/` 아래의 심볼릭 링크로 노출하는데, 기존 경로 로직이 패키지 루트가 아니라 `.bin`을 가리켜서 `npx career-ops-ui init`이 `MODULE_NOT_FOUND`로 죽었습니다. 이제 `bin/career-ops-ui.sh`와 `bin/start.sh`는 심볼릭 링크 체인을 따라 스크립트 경로를 해석하므로(`readlink` + `cd -P`) 모든 동사가 저장소에서, `npm link`로, 그리고 `npx`로 동작합니다. `.bin` 스타일 심볼릭 링크로 동사를 실행하는 새 회귀 테스트로 고정했습니다.
>
> _전체 스위트 **1065/1065** 통과 · i18n + 문서를 9개 로케일에 동기화._
