...
> **v1.67.0 — `#/scan`에 급여 범위 필터 추가.** 결과 테이블에 텍스트·원격 필터 옆으로 숫자 입력 두 개(**급여 최소 / 최대**)가 추가됩니다. 각 행의 자유 텍스트 급여(`от 100 000 до 200 000 ₽`, `120000-150000 USD`, `$120K–$150K` 등)를 숫자 범위로 파싱해 범위 겹침 방식으로 매칭합니다. 급여가 없는 행은 계속 표시되므로 필터가 목록을 비우지 않고 좁힙니다(통화 구분 없음 — 환율 변환 없음). 또한 소스별 fetch 타임아웃을 **15s → 30s**로 상향해 느린 Ashby 보드의 타임아웃을 해소. 전체 스위트 **1060/1060** 통과.
