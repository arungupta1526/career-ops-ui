...
> **🆕 최신 릴리스 — v1.83.0**
>
> **재게시 / 유령 공고 감지기**(상위 career-ops v1.15.0 패리티): `#/scan`에 새로운 **🔁 재게시 / 유령 공고** 패널이 추가되어, 90일 롤링 윈도우 내에서 다른 URL로 재게재된 회사+직무 클러스터를 표시합니다 — 오래된 채용 파이프라인 / 유령 공고 신호 — 스캔 기록에서 읽어옵니다(`GET /api/scan/reposts`). v1.82.0(NoDesk), v1.81.0(13개 신규 소스), v1.80.0(Teamtailor, 소스 격리, 저장된 검색)을 기반으로 합니다.
>
> _13 locales · 6개 LLM 프로바이더 · 41개 스캐너 어댑터 · 재게시 감지기 · 상위 career-ops v1.15.0 패리티._
