...
> **🆕 최신 릴리스 — v1.79.0**
>
> **WeWorkRemotely 스캔 소스(상위 career-ops v1.14.0 패리티).** 보드 전체를 아우르는 [We Work Remotely](https://weworkremotely.com)의 리모트 채용 RSS 피드가 이제 정식 스캔 소스입니다 — `provider: weworkremotely` 항목을 추가하면 `#/scan`의 **Source** 드롭다운에 나타납니다(총 **26개 어댑터**). 호스트 고정 + `redirect:'error'`(SSRF 안전); 제목은 `Company: Role` 기준으로 분리됩니다. 또한 `title_filter` 키워드는 길이 검사 전에 트리밍됩니다(상위 #1261). v1.78.x(스캔 **국가 필터**, 결과 **자동 새로고침**, 전역 검색 **Enter→스캔**, 클릭 가능한 로고), v1.77.0(덴마크어, 13번째 로케일), v1.76.0(테넌트별 ATS 소스 6종, `trust_filter`, 무제한 스캔)을 기반으로 합니다.
>
> _13 locales · 6개 LLM 프로바이더 · 26개 스캐너 어댑터 · 국가 필터 · 상위 career-ops v1.14.0 패리티._
