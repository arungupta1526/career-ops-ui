...
> **🆕 최신 릴리스 — v1.82.0**
>
> **NoDesk 스캔 소스**(상위 career-ops v1.15.0 패리티): 보드 전체 NoDesk 원격 채용 RSS 피드가 이제 1급 스캔 소스입니다 — `provider: nodesk` 항목을 추가하면 `#/scan`의 **Source** 드롭다운에 표시됩니다(총 **41개 어댑터**). 호스트는 `nodesk.co`로 고정되고 `redirect:'error'`(SSRF 방지); 제목은 `Role at Company`로 분리됩니다. v1.81.0(13개 신규 소스), v1.80.0(Teamtailor, 소스 격리, 저장된 검색), v1.77.0(덴마크어)을 기반으로 합니다.
>
> _13 locales · 6개 LLM 프로바이더 · 41개 스캐너 어댑터 · 상위 career-ops v1.15.0 패리티._
