...
> **v1.66.0 — RU 소스가 모든 결과 페이지를 순회.** hh.ru, Habr Career, Trudvsem은 쿼리당 처음 ~50건만 가져왔지만, 이제 끝까지 페이지를 따라갑니다 — hh.ru/Habr는 `&page=N`, Trudvsem은 `offset`/`meta.total` — 페이지 간 중복 제거 및 새 항목이 없으면(또는 50페이지 안전 상한에서) 중단합니다. "Backend разработчик" 같은 쿼리가 한 페이지가 아니라 전체 결과를 반환합니다(예: hh.ru PHP **17 → 55건 이상**을 3페이지에 걸쳐, Trudvsem은 72건 전부). 페이지별 요청은 기존 타임아웃 + `AbortSignal`을 유지. 전체 스위트 **1045/1045** 통과.
