...
> **🆕 최신 릴리스 — v1.67.1**
>
> **1. `#/scan` 급여 범위 필터.** 결과 테이블에 텍스트·원격 필터 옆으로 숫자 입력 두 개(**급여 최소 / 최대**)가 추가됩니다. 각 행의 자유 텍스트 급여(`от 100 000 до 200 000 ₽`, `120000-150000 USD`, `$120K–$150K` 등)를 숫자 범위로 파싱해 범위 겹침 방식으로 매칭합니다 — 행의 급여 범위가 `[최소, 최대]` 창과 겹치면 유지됩니다. **급여가 없는 행은 계속 표시되므로** 필터가 목록을 비우지 않고 좁힙니다. 비교는 **통화를 구분하지 않습니다**(환율 변환 없음).
>
> **2. 소스별 fetch 타임아웃을 10s로 설정(fail-fast).** v1.67.0에서 30s로 올려도 느린 Ashby 보드의 절반 정도만 회복했고, 나머지(Perplexity, Supabase, Resend, DeepL, Ramp 등)는 마감과 무관하게 멈추므로 긴 타임아웃은 죽은 슬롯을 기다리며 스캔을 지연시킬 뿐이었습니다. 10s는 만성 행에 즉시 실패하고 스캔 반응성을 유지합니다. `SCAN_FETCH_TIMEOUT_MS`로 재정의 가능.
>
> _전체 스위트 **1060/1060** 통과 · i18n + 문서를 9개 로케일에 동기화._
