...
> **🆕 최신 릴리스 — v1.78.0**
>
> **스캔 페이지의 지리 필터 — 채용 결과를 국가별로, 국기와 함께 필터링합니다.** `#/scan`의 새 **Country** 드롭다운은 스캔된 결과에서 감지된 모든 국가를 나열하며(국기 이모지 + 개수), 특정 국가에 묶인 역할만 남길 수 있습니다 — Remote / Hybrid / Onsite 근무 형태 필터와 나란히 제공되어, 국가에 묶인 일자리**와** 원격 일자리를 모두 검색할 수 있습니다. 새 `countries.js` 헬퍼는 각 공고의 자유 텍스트 위치(국가명, 별칭, ~100개 주요 채용 시장 도시)를 ISO 국가 + 국기로 매핑합니다. 감지는 보수적이며 결코 추측하지 않습니다. v1.77.0(덴마크어, 13번째 로케일)과 v1.76.0(상위 career-ops v1.13.0 패리티 — 테넌트별 ATS 소스 6종, `trust_filter`, 무제한 스캔 결과)을 기반으로 합니다.
>
