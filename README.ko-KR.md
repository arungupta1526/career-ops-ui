...
>
> **2. 소스별 fetch 타임아웃을 15s → 30s로 상향**(`SCAN_FETCH_TIMEOUT_MS`로 재정의 가능). Ashby의 `includeCompensation` 페이로드는 ×8 동시성에서 흔히 15s를 초과해 매 스캔마다 ~30개 Ashby 보드가 타임아웃되었습니다. 30s는 느리지만 살아있는 소스가 완료되도록 합니다.
>
> _테스트 13개 추가 · 전체 스위트 **1060/1060** 통과 · i18n + 문서를 9개 로케일에 동기화._
