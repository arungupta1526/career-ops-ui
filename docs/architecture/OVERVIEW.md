...
- **`lib/llm-output.mjs`** *(v1.58.0)* — `cleanLlmMarkdown(md)`: strips agent/tool scaffolding the model sometimes echoes into prose (`<tool_call>{…}</tool_call>`, `<tool_response>`, `<tool_use>`, `<function_call>`, `<thinking>`, `[TOOL_CALL]…`, and a dangling unterminated trailing tag). Pure + idempotent. Applied at every provider boundary (anthropic/openai/qwen/openrouter), the Gemini-subprocess path, and on serving an already-saved interview-prep brief. **Not** an HTML sanitizer — `UI.md()` remains the XSS boundary; this only declutters the document.
