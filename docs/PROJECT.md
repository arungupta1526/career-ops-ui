...
- In-process portal scanners — **19 adapters**: 14 EN-region (Greenhouse / Ashby / Lever / Workable / SmartRecruiters / Workday + RSS, plus the v1.75.0 aggregators RemoteOK / Remotive / Working Nomads / IBM / Arbeitsagentur / Glints / Jobstreet · SEEK) + 5 RU portals (hh.ru / Habr Career / Trudvsem / GetMatch / GeekJob). All bypass Playwright. Discoverable via `GET /api/scan/sources`. Since v1.69.0 (P-14) adding another = dropping a `<slug>.mjs` with a `meta` export into `server/lib/sources/`; the registry auto-discovers it (no registry edit). The seven v1.75.0 aggregators are selected by an explicit `provider:` field rather than a `careers_url`; the config-driven four (IBM / Arbeitsagentur / Glints / Jobstreet · SEEK) read a per-entry `<provider>:` block threaded through as `opts.company`.
