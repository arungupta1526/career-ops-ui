...
- Current count as of **v1.72.0**: **1116** `node --test` cases (unit + functional + acceptance) + Playwright/E2E surfaces (smoke + full-cycle + forms + locale-sweep ×12 + theme-toggle) + the shell-surface tier (`tests/sh-files.test.mjs` — `bin/*.sh` + `.githooks` + `install-hooks` wiring, WS9). Run `npm run test:coverage` for the V8 report; see `docs/architecture/TESTING.md` for the 4-tier pyramid.
