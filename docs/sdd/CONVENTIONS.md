...
- Help-bundle markdown lives in `docs/help/<locale>.md` (**12 files**: `en, es, pt-BR, ko-KR, ja, ru, zh-CN, zh-TW, fr, pl, uk, ar` — pl/uk/ar added & fully translated in v1.71.0). CI invariant: **19 H2 sections** (`tests/canonical-docs-coverage.test.mjs` + `tests/help-ui.test.mjs` `SECTION_COUNT`) and **75 H3** (`tests/help-ru-config-section.test.mjs`). §19 "Localizing the app into your language" was added in v1.60.0.
...
- Current count as of **v1.71.0**: **1100** `node --test` cases (unit + functional + acceptance) + Playwright/E2E surfaces (smoke + full-cycle + forms + locale-sweep ×12 + theme-toggle) + the shell-surface tier (`tests/sh-files.test.mjs` — `bin/*.sh` + `.githooks` + `install-hooks` wiring, WS9). Run `npm run test:coverage` for the V8 report; see `docs/architecture/TESTING.md` for the 4-tier pyramid.
