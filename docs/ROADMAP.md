...
- **`v1.56.1 → v1.58.2` — FIX-PROMPT-FINAL + provider + external-QA cycle (May 2026)** — one-fix-per-release, every release CI-green + pre-commit AI-review LGTM, each regression-locked:
  ...
  - Net: **896** `node --test` + **58** Playwright + 20/23 E2E; 8-locale CHANGELOG/README/help parity; `qa/v158-regression/{FIX-PROMPT,REGRESSION-FINAL}` + exhaustive matrices appended to the 3 perennial prompts. Sole open item: **G-005** (cross-repo, blocked on parent).
