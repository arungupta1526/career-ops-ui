...
### Скан hh.ru из-за рубежа (`HH_PROXY`)

hh.ru гео-блокирует свой публичный API по **IP-адресу**. С не-российского узла каждый запрос к `api.hh.ru` возвращает **HTTP 403 forbidden**; сканер логирует предупреждение, отключает hh.ru на этот прогон и идёт дальше. Один `HH_USER_AGENT` блок **не снимает**, и «браузерные» заголовки (Referer, Accept-Language и пр.) тоже — решает **IP**, и только он. Чтобы включить hh.ru из-за рубежа, направьте **только его запрос** через российский прокси переменной `HH_PROXY`; остальные источники остаются на прямом соединении.

**1 — Возьмите российский прокси. Три независимых барьера, нужны все:**
- **IPv4, не IPv6.** У `api.hh.ru` нет IPv6 (нет AAAA). IPv6-прокси до него не достучится — каждый раз `502 Bad Gateway / Host Not Found`. IPv6 идёт по умолчанию и дешевле, поэтому явно берите **IPv4**.
- **HTTP/HTTPS, не SOCKS.** Прокси туннелирует HTTPS через `CONNECT`. **SOCKS5 не поддерживается** (реализация на `ProxyAgent` из undici).
- **Residential/мобильный, не датацентр.** hh.ru отдаёт **403** для хостинговых/датацентровых диапазонов *даже если IP российский* (Selectel, Timeweb, VDSina…). Нужен российский **резидентский или мобильный** прокси — реальный IP от ISP. Берите *резидентский / мобильный*, а не *серверный*. Оговорка: многие перепродаваемые residential-**пулы тоже детектятся и ловят 403** — в тестах РФ-датацентр, РФ-residential *пул* и полные браузерные заголовки всё равно дали 403. Надёжнее всего — **собственное домашнее российское подключение** сервера или OAuth-токен зарегистрированного приложения **dev.hh.ru**.

**2 — Проверьте до подключения.** С сервера:
```bash
curl -x "http://LOGIN:PASS@HOST:PORT" \
  "https://api.hh.ru/vacancies?text=php&per_page=1" -w "\nHTTP %{http_code}\n"
```
- `HTTP 200` + JSON → работает, продолжайте.
- `HTTP 403 {"type":"forbidden"}` → доходит до hh.ru, но IP заблокирован → это датацентр-прокси, нужен residential/мобильный.
- `CONNECT tunnel failed, response 502` → не доходит до hh.ru (IPv6-прокси, истёкший тариф или IP не в whitelist).
- `407` → неверные креды.

**3 — Пропишите.** Добавьте в `.env` в **корне проекта career-ops** (там же, где `HH_USER_AGENT`):
```
HH_PROXY=http://LOGIN:PASS@HOST:PORT
```
Читается при старте — после правки **перезапустите сервер**. В `.env` лежит пароль; не коммитьте его.

**4 — Проверьте.** Перезапустите скан на `#/scan`; строки hh.ru появятся рядом с Habr Career. Всё ещё пропускается? Повторите `curl` из шага 2 — ломается прокси, а не приложение.
