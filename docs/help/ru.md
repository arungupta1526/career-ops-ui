...
для разработки (Claude Code, Codex, Cursor, Gemini CLI, GitHub
Copilot CLI). Модель-агностична. Оценивает каждую вакансию против
...
Workday (английские доски) и hh.ru / Habr Career (российские доски,
если включены).
...
```yaml
russian_portals:
  sources: [hh, habr]      # или только один
