...
### 在俄罗斯境外扫描 hh.ru（`HH_PROXY`）

hh.ru 按 **IP 地址**对其公开 API 进行地域封锁。从非俄罗斯出口节点发出的每个 `api.hh.ru` 请求都会返回 **HTTP 403 forbidden**；扫描器记录警告，在本次运行中禁用 hh.ru，并继续其他来源。仅靠 `HH_USER_AGENT` **无法**解除封锁，浏览器式请求头（Referer、Accept-Language 等）也不行——关卡是你的 **IP**。要在境外纳入 hh.ru，用 `HH_PROXY` 仅将**它的请求**经俄罗斯代理转发；其余来源保持直连。

**1 — 准备一个俄罗斯代理。三个独立条件，缺一不可：**
- **IPv4，而非 IPv6。** `api.hh.ru` 仅支持 IPv4（无 AAAA 记录）。IPv6 代理永远无法到达——每次都 `502 Bad Gateway / Host Not Found`。请明确选择 **IPv4**。
- **HTTP/HTTPS，而非 SOCKS。** 代理通过 `CONNECT` 隧道传输 HTTPS。**不支持 SOCKS5**（实现使用 undici 的 `ProxyAgent`）。
- **住宅/移动，而非数据中心。** *即便 IP 是俄罗斯的*，hh.ru 也会对主机托管/数据中心网段返回 **403**（Selectel、Timeweb…）。你需要俄罗斯**住宅或移动**代理（真实 ISP 分配的 IP）。
...
```
- `HTTP 200` + JSON → 可用。
- `HTTP 403 {"type":"forbidden"}` → 到达 hh.ru 但 IP 被封 → 是数据中心代理，改用住宅/移动。
- `CONNECT tunnel failed, response 502` → 无法到达 hh.ru（IPv6 代理、套餐过期或 IP 未加白名单）。
- `407` → 凭据错误。

**3 — 接入。** 在 **career-ops 项目根目录**的 `.env`（与 `HH_USER_AGENT` 同一文件）中添加：
```
HH_PROXY=http://LOGIN:PASS@HOST:PORT
```
启动时读取——编辑后请**重启服务器**。`.env` 含密码；切勿提交。

**4 — 验证。** 在 `#/scan` 重新扫描；hh.ru 的条目会出现在 Habr Career 旁边。
