...
User opens `#/scan`, optionally narrows results by source (11 entries — 6 EN ATS + 5 RU as of v1.29.0), clicks **🌐 Scan**. The server streams SSE events as adapters work in two sequential phases (ATS → Regional), hits accumulate into a paginated results table, and the user can filter / page through the full set.
...
- `GET /api/scan/sources` — canonical source registry (`server/lib/sources/registry.mjs`). Returns 11 entries with `{ value, label, region, configKey? }`. The `#/scan` source-filter dropdown rebuilds on mount from this endpoint. Adding a 12th adapter = one entry in the registry.
...
- Filters in the SPA table:
  ...
  - **Source** dropdown — built dynamically from `GET /api/scan/sources` (v1.29.0). Default 11 entries, alphabetical by label: Ashby · GeekJob · Greenhouse · GetMatch · Habr Career · hh.ru · Lever · SmartRecruiters · Trudvsem · Workable · Workday.
