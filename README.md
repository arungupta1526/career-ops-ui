...
> **🆕 Latest release — v1.76.0**
>
> **Parent career-ops v1.13.0 parity — six new job sources + scanner hardening.** Six per-tenant ATSes (**BambooHR**, **Breezy HR**, **Comeet**, **Personio**, **Recruitee**, **SolidJobs**) auto-detect from a `careers_url` host and are now selectable in `#/scan` (25 adapters total). Plus: the scan results table **no longer caps** its stored rows (`MAX_STORED_RESULTS` removed — page through everything); **title-filter robustness** (short acronyms like COO/SDR match on word boundaries, malformed-config guard); config-driven Arbeitsagentur **`remoteMatch`** (`title` / `filter` server-side homeoffice + pagination / `off`); and an optional **`trust_filter`** that annotates each posting with a trust score/level/flags (⚠ badge in `#/scan`, never drops a job). Builds on v1.75.0 (seven aggregator sources) and v1.70–74 (12 locales incl. Arabic RTL, 6 LLM providers).
>
> _12 locales · 6 LLM providers · 25 scanner adapters · parent career-ops v1.13.0 parity._
