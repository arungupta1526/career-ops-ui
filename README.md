...
> **v1.59.12 — i18n dictionary cleanup ahead of the French locale PR.** Removed a personal-data placeholder, restored the `followup` date placeholder to a format hint, and added `npm run audit:i18n` (CI-wired) that hard-fails on personal data, locale-parity gaps, empty values, and rotting calendar dates. Coincidental duplicate-value keys (`nav.scan` vs `scan.col.company`) are intentional — kept for per-role translation flexibility. Full QA hand-off in [`qa/QA-REGRESSION-PROMPT.md`](qa/QA-REGRESSION-PROMPT.md).
...
This clones both repos (career-ops + career-ops-ui), installs deps, runs the doctor, and starts the server at http://127.0.0.1:4317.

**Or step-by-step with the `career-ops-ui` CLI** (clone → setup → init → run):
...
The provider you pick sets `LLM_PROVIDER` (`auto` | `claude` | `gemini`); the server's live evaluate / deep-dive / mode / auto-pipeline routes honour it. You can change it any time from **`#/config` → API keys** without restarting.
...
```bash
npm test                       # 813 unit/integration tests
npm run test:e2e               # 20 smoke e2e (boots own server)
npm run test:e2e:full          # 23 comprehensive e2e
npm run test:e2e:browser       # 32 Playwright browser (smoke + full-cycle)
npm run test:coverage          # same as `npm test` plus V8 coverage
```

| Suite                       | Tests | What                                                                                                       |
| --------------------------- | ----- | ---------------------------------------------------------------------------------------------------------- |
| `node --test tests/*.test.mjs` (unit + integration) | **813** | Every endpoint, ephemeral server, no network. 110 files: parsers, scanners (mocked), runners, anthropic/openai, security headers, XSS, JD sanitize, URL validation, i18n parity, + the v1.55→v1.56 UX-fix suites. |
...
| `npm run test:e2e:browser` (`playwright-smoke` + `playwright-full-cycle`) | **32** | Browser-driven: dashboard render, navigation, language switch, 404, health, tracker round-trip, pipeline add + invalid-URL sweep, reports, evaluate manual fallback, config keys masked, CV PUT XSS strip, pipeline preview 400, auto-pipeline SSE. |
| **Total**                   | **888** | **0 fails, 0 flakes**                                                                                    |
