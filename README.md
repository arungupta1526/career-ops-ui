...
> **🆕 Latest release — v1.84.0**
>
> **Re-apply cooldown + compensation in pipeline.md** (parent career-ops v1.15.0 parity): the EN scan now **skips roles at companies you applied to recently** (`config/profile.yml → re_apply_windows:`, off by default) so results stay focused on new openings, and scanned offers persist their **salary** as an optional `url | <comp>` column in `data/pipeline.md`. Builds on v1.83.0 (repost detector), v1.82.0 (NoDesk), and v1.81.0 (13 new sources).
>
> _13 locales · 6 LLM providers · 41 scanner adapters · repost detector · re-apply cooldown · parent career-ops v1.15.0 parity._
