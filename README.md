...
>
> **WeWorkRemotely scan source (parent career-ops v1.14.0 parity).** The board-wide [We Work Remotely](https://weworkremotely.com) remote-jobs RSS feed is now a first-class scan source — add a `provider: weworkremotely` entry and it appears in the `#/scan` **Source** dropdown (**26 adapters** total). Host-pinned + `redirect:'error'` (SSRF-safe); titles split on `Company: Role`. Also: `title_filter` keywords are trimmed before the length check (parent #1261). Builds on v1.78.x (Scan **country filter**, results **auto-refresh**, global-search **Enter→Scan**, clickable logo), v1.77.0 (Danish, 13th locale), and v1.76.0 (six per-tenant ATS sources, `trust_filter`, uncapped scan).
>
> _13 locales · 6 LLM providers · 26 scanner adapters · country filter · parent career-ops v1.14.0 parity._
