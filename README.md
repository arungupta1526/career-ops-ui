...
> **🆕 Latest release — v1.74.3**
>
> **Parent project now tracks the [`Fighter90/career-ops`](https://github.com/Fighter90/career-ops) fork.** Every live reference to the parent — the `bin/setup.sh` installer's clone default, the `git clone` / onboarding / "sits on top of" links across all 12 READMEs, and the agent docs (`CLAUDE.md`, `AGENTS.md`, `GEMINI.md`, `docs/`) — now points at the maintained fork instead of upstream. Author credit to santifer (`santifer.io`) and the unofficial-UI disclaimer are unchanged; only source/clone URLs moved. Builds on v1.74.0 (6 AI assistants incl. GitHub Models) and v1.70–73 (12 locales incl. Arabic RTL, cover-letter + PDF, verified provider context).
>
> _Full suite **1134/1134** green · 12 locales · 6 LLM providers · parent source → fork._
...
[career-ops](https://career-ops.org) is an open-source job-search system that runs as slash commands inside any AI coding CLI (Claude Code, Gemini CLI, Codex, Qwen Code, OpenCode, GitHub Copilot CLI — other Claude-compatible CLIs work too via the same slash-command surface). Model-agnostic. It evaluates each posting against your CV with a six-dimension 0.0–5.0 rubric, generates tailored PDF résumés, and tracks every application locally — no cloud accounts, no telemetry, no auto-submit.
