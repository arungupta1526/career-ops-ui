...
>
> **NoDesk scan source** (parent career-ops v1.15.0 parity): the board-wide [NoDesk](https://nodesk.co) remote-jobs RSS feed is now a first-class scan source — add a `provider: nodesk` entry and it shows up in the `#/scan` **Source** dropdown (**41 adapters** total). Host-pinned to `nodesk.co` with `redirect:'error'` (SSRF-safe); titles split on `Role at Company`. Builds on v1.81.0 (13 new sources — Arbeitnow / Himalayas / Jobicy / Landing.jobs / 4 Day Week / The Muse / The Hub / Jobspresso / Hacker News / JustJoin.it / NoFluffJobs / Pinpoint / Rippling), v1.80.0 (Teamtailor, source quarantine, saved searches), and v1.77.0 (Danish).
>
> _13 locales · 6 LLM providers · 41 scanner adapters · parent career-ops v1.15.0 parity._
