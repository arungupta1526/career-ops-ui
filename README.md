...
> **🆕 Latest release — v1.69.0**
>
> **Plug-in scanner auto-discovery (P-14) — drop a `.mjs` into `server/lib/sources/` and it registers itself.** Before v1.69 the source list in `server/lib/sources/registry.mjs` was a hand-maintained array: adding an adapter meant editing both `<id>.mjs` AND `registry.mjs`. Now every adapter declares a self-describing `export const meta = { value, label, region, configKey? }` block, and the registry auto-discovers it at boot (`readdirSync` + dynamic `import()`). Closes the partial half of roadmap item P-14. The public API (`SOURCES`, `SOURCES_BY_REGION`, `RU_CONFIG_KEYS`, `getRegionalSources`) is unchanged — every existing import keeps working, and malformed `meta` is skipped with a single diagnostic warning. Help §17 ("How to add a new job-portal source") rewritten across all 9 locales for the drop-in flow.
>
> _Full suite **1079/1079** green · i18n + docs synced across all 9 locales._
