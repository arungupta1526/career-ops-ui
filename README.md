...
> **🆕 Latest release — v1.67.1**
>
> **1. Salary range filter on `#/scan`.** The scan results table gains two numeric inputs — **salary from / to** — beside the text and remote filters. Each row's free-text pay (`от 100 000 до 200 000 ₽`, `120000-150000 USD`, `$120K–$150K`, …) is parsed to a numeric range and matched with overlapping-range semantics — a row is kept when its pay range overlaps your `[from, to]` window. Rows with **no published salary stay visible**, so the filter narrows rather than guts the list. Comparison is **currency-agnostic** (no FX conversion — a `₽` figure and a `$` figure with the same digits compare equal).
>
> **2. Per-source scan fetch timeout set to 10s (fail-fast).** Raising it to 30s in v1.67.0 recovered only ~half the slow Ashby boards — the rest (Perplexity, Supabase, Resend, DeepL, Ramp, …) hang regardless of the deadline, so a longer timeout just stalled every scan waiting on dead slots. 10s fails fast on the chronic hangers and keeps the scan responsive; override with `SCAN_FETCH_TIMEOUT_MS`.
>
> _Full suite **1060/1060** green · i18n + docs synced across all 9 locales._
