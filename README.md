...
> **🆕 Latest release — v1.69.1**
>
> **Two fixes.** **(1) scan:** the `#/scan` results table no longer silently truncates large regional sweeps — the stored/displayed match set was hard-capped at 500 per region (a real RU scan of 1352 matching jobs showed only 500; 852 hidden — the "2000 scanned, ~600 shown" symptom). Both scanners now cap at a shared, env-overridable `MAX_STORED_RESULTS` (default 2000, override via `SCAN_MAX_RESULTS`); display-only, adding to `pipeline.md`/scan-history already used the uncapped new-offers set. **(2) health/ui:** `#/health` check cards no longer overflow — a long name/value collided with the Fix button + badge; the row now shrinks/wraps via `.health-check-row`.
>
> _Full suite **1084/1084** green · i18n + docs synced across all 9 locales._
