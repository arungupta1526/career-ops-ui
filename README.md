...
> **v1.59.11 — 26-release cycle closed.** All four CI jobs (Unit + integration Node 18 / 20 / 22 + Playwright e2e) `success` for the first time this cycle. Definitive QA hand-off in [`qa/QA-REGRESSION-PROMPT.md`](qa/QA-REGRESSION-PROMPT.md) with §−1 probe-methodology guards so the next tester doesn't repeat false-negatives (e.g. `fetch`-client-side URL normalisation hiding the server's `..` guard).
