...
>
> **2. Per-source scan fetch timeout raised 15s → 30s** (override with `SCAN_FETCH_TIMEOUT_MS`). Ashby's `includeCompensation` payloads routinely took longer than 15s under 8-way concurrency, so ~30 Ashby boards were timing out on every scan; 30s lets slow-but-alive sources finish while still capping a genuinely dead upstream.
>
> _13 new tests · full suite **1060/1060** green · i18n + docs synced across all 9 locales._
