...
> **🆕 Latest release — v1.80.0**
>
> **Five scan upgrades** (ideas from [job-crawler](https://github.com/bracketouverte/job-crawler), reimplemented): a **Teamtailor** source — per-tenant `<slug>.teamtailor.com` sites via their public `/jobs.rss` feed (**27 adapters** total); **source quarantine** — dead 404/410 sources are recorded and skipped on later scans (self-healing after 14 days), killing the recurring dead-slug noise; a **Max per source** cap field (∞ by default); a **Posted within** age filter (24h / 7d / 30d); and **saved searches + ★ favorites** on `#/scan`, persisted in `localStorage` with defensive validation. Builds on v1.79.0 (We Work Remotely), v1.78.x (country filter, auto-refresh, Enter→Scan, clickable logo), v1.77.0 (Danish), and v1.76.0 (per-tenant ATS sources, `trust_filter`).
>
> _13 locales · 6 LLM providers · 27 scanner adapters · saved searches + favorites · parent career-ops v1.14.0 parity._
