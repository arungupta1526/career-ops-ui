...
> **🆕 Latest release — v1.68.2**
>
> **`npx` / `npm link` CLI verbs were broken — now fixed.** npm and npx expose `career-ops-ui` as a symlink under `node_modules/.bin/`, where the old path logic pointed at `.bin` instead of the package root — so `npx career-ops-ui init` died with `MODULE_NOT_FOUND`. `bin/career-ops-ui.sh` and `bin/start.sh` now resolve the script path through the symlink chain (`readlink` + `cd -P`), so every verb works from the repo, via `npm link`, and via `npx`. Locked by a new regression test that runs a verb through a `.bin`-style symlink.
>
> _Full suite **1065/1065** green · i18n + docs synced across all 9 locales._
