...
> **v1.67.0 — salary range filter on `#/scan`.** The scan results table gains two numeric inputs — **salary from / to** — beside the text and remote filters. Each row's free-text pay (`от 100 000 до 200 000 ₽`, `120000-150000 USD`, `$120K–$150K`, …) is parsed to a numeric range and matched with overlapping-range semantics; rows with no published salary stay visible, so the filter narrows rather than guts the list (currency-agnostic — no FX). This release also raises the per-source scan fetch timeout **15s → 30s** so slow Ashby boards stop timing out. Full suite **1060/1060** green.
